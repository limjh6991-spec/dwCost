...
export default {
  ...
  yearOptions: Array.from({length: 7}, (_, i) => new Date().getFullYear() - 3 + i),
      showYyyymmPopup: false,
      params: {
        yyyymm: null,
  site: 'HQ',
      },
      ...
      isProcessing: false,
  duplicateKey: ['yyyymm', 'selCode', 'site','dwModel'],
  ...
  methods: {
    selectMonth(month) {
      ...
      this.updateYyyymm();
      this.showYyyymmPopup = false;
    },
    toggleYyyymmPopup() {
      this.showYyyymmPopup = !this.showYyyymmPopup;
    },
    selectMonthPopup(month) {
      this.selectedMonth = month;
      this.updateYyyymm();
      this.showYyyymmPopup = false;
      // 항상 'YYYY-MM' 형태로 세팅
      this.params.yyyymm = `${this.selectedYear}-${month}`;
    },
    initialize() {
  ...
  this.params.site = this.userAuthInfo.curProdCtg === 'VN' ? 'VINA' : '본사';
    ...
    updateYyyymm() {
      // 항상 'YYYY-MM' 형태로 세팅
      this.params.yyyymm = `${this.selectedYear}-${this.selectedMonth}`;
    },
    initializeGrid() {
  ...
  site: this.siteMap[this.params.site],
    ...
    uploadClick() {
      ...
      this.$refs.uploadPopup1.openDialog({
        dialogTitle: '업로드 팝업',
        uploadApi: '/api/c0007000/C0007003/upload',
    ...
    */
    async excelBtnClick() {
      ...
      const fileName = `생산정보${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
