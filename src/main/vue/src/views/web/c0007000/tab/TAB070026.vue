  ...
  <div>
    ...
    <div class="grid_box search_onerow">
      ...
      <div class="grid-border-none">
        <RealGrid ref="bizStockGrid" :uid="'bizStockGrid'" :step="'1'" :rows="gridRows" style="height: 100%" :fixLayoutWidth="false" />
      </div>
    </div>
  </div>
</template>
...
import gridField from '@web/c0007000/js/TAB070026.js';
...
export default {
  ...
  computed: {
    gridView() { return this.$refs.bizStockGrid?.getGridView(); },
  },
  created() { this.bizStockGrid = _.cloneDeep(gridField); },
  mounted() {
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const rows = [];
      await this.$axios.api.search({ menuId: 'c0007015', queryId: 'BizStockSum_Sch1', queryParams: { site: this.siteMap[this.params.site] }, target: rows });
    ...
    apiCallClick() {
      ...
      this.callIface({
        ...
        yyyymm: yyyymm,
        params: {
          BizUnit: 0, DateFr: `${yyyymm}01`, DateTo: `${yyyymm}${lastDay}`,
          SMQryType: 'S', SMUnitType: '8103001', ConvUnitSeq: '', AssetSeq: '', SMStatus: '',
          ItemClassSSeq: '', ItemName: '', ItemNo: '', Spec: '', IsZeroQty: '0',
          ItemClassLSeq: '', ItemClassMSeq: '', MultiBizUnit: '',
