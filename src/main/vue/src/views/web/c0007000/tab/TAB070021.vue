...
import gridField from '@web/c0007000/js/C0007013.js';
...
export default {
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const rows = [];
      await this.$axios.api.search({ menuId: 'c0007013', queryId: 'C0007013_Sch1', queryParams: params, target: rows });
    ...
    delBtnClick() {
      ...
      this.$confirm('확인', `${checkedRows.length}건을 삭제하시겠습니까?`, async (confirmed) => {
        ...
        if (existingRows.length > 0) {
          try {
            await this.$axios.api.saveData({ menuId: 'c0007013', delete: [{ queryId: 'C0007013_Delete1', data: existingRows }] });
            this.searchClick();
