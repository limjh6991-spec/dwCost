 ...
 * 컬럼이 많아 기타입고/기타출고/재고 그룹은 +/- 로 여닫음 (제조/제품원가 탭 참조)
 */
<template>
  <div>
...
export default {
  ...
  computed: {
    ...
    gridView() {
      return this.$refs.prodSubulGrid?.gridView;
    },
    gridDataProvider() {
      return this.$refs.prodSubulGrid?.dataProvider;
    },
  },
  created() {
    this.initialize();
  ...
  methods: {
    ...
    excelBtnClick() {
      ...
      const fileName = `생산실적_월별집계_수량VN_${yyyymmdd}.xlsx`;
