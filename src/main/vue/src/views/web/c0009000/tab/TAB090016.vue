...
export default {
  ...
  watch: {
    ...
    prodCtg: {
      handler(newVal) {
        if (newVal && this.$refs.wipCostGrid != null) {
          this.initialize();
          this.getDataList();
        }
      },
  ...
  computed: {
    ...
    gridView() { return this.$refs.wipCostGrid ? this.$refs.wipCostGrid.getGridView() : null; },
  },
  created() {
  ...
  methods: {
    ...
    excelBtnClick() {
      ...
      gv.exportGrid({
        type: 'excel', target: 'local', fileName: `매출원가_제품_VN_${yyyymmdd}.xlsx`,
