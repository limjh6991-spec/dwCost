  ...
  <div>
    ...
    <div class="grid_box search_onerow">
      ...
      <div class="grid-border-none">
        <RealGrid ref="deptGrid" :uid="'deptGrid'" :step="'1'" :rows="deptGridRows" style="height: 100%" />
      </div>
    </div>
...
import gridField from '@web/c0001000/js/C0001002.js';
...
export default {
  ...
  computed: {
    gridView() {
      return this.$refs.deptGrid.getGridView();
    },
    gridDataProvider() {
      return this.$refs.deptGrid.getGridDataProvider();
    },
  },
  created() {
    this.initialize();
  ...
  methods: {
    ...
    uploadClick() {
      ...
      this.$refs.uploadPopup1.openDialog({
        dialogTitle: '업로드 팝업',
        uploadApi: '/api/c0001000/c0001002/upload',
        headers: ['field1', 'field2', 'field3', 'field4', 'field5', 'field6', 'field7', 'field8'],
        excelGrid,
        fileName: '부서코드정보_template',
      });
    },
    closePopup() {
      this.searchClick();
    ...
    async excelBtnClick() {
      ...
      const fileName = `부서코드정보${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
    ...
    onValidateColumnDeptGrid(grid, column, inserting, value, itemIndex, dataRow) {
      ...
      if (this.$utils.containsValue(['yyyy', 'selCode', 'site', 'dept', 'deptName', 'expenArea'], column.fieldName)) {
        if (_.isNil(value)) {
          error.level = 'error';
      ...
      if (this.duplicateIndices.includes(itemIndex) && this.$utils.containsValue(['yyyy', 'selCode', 'site', 'dept'], column.fieldName)) {
        error.level = 'warning';
