...
import com.dowinsys.cost.web.c0007000.mapper.C0007003Mapper;
import com.dowinsys.cost.web.c0007000.service.C0007003Service;
...
@Service("com.dowinsys.cost.web.c0007000.service.C0007003")
public class C0007003ServiceImpl implements C0007003Service {

    @Autowired
    C0007003Mapper mapper;
        ...
        try {
            ...
            if (!duplicateOrgList.isEmpty() || !duplicateList.isEmpty() || !pkDuplicateList.isEmpty()) {
                ...
                for (Map<String, String> item : duplicateOrgList) {
                    acctList.add(item.get("modelNType"));
                }
                ...
                if (!duplicateOrgList.isEmpty()) {
                    ...
                    errorMessage.append("MODEL_N_TYPE는 동일년월 동일사업장에 이미 존재하는 데이터로 업로드 대상이 아닙니다.");
