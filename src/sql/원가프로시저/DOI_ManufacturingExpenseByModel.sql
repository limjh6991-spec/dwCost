CREATE                         Procedure DOI_ManufacturingExpenseByModel(
	@YYYYMM varchar(10),
 	@SITE varchar(4),
 	@SEL_CODE varchar(10)
 )
...
/*		union all select 90 rn, '      1. 제)견본비' gubun*/
	...
	left join (
		...
		and ((mat_class+coalesce(자재대분류,'1') ='약액'+'1'  ) or (mat_class+자재대분류 ='부자재'+'약액' ))
		group by 구분,도우모델		
		union all 
		...
		and (mat_class+coalesce(자재대분류,'1') in ('부자재'+'1','부자재'+'부자재','부자재'+'카세트','더미글라스'+'부자재','부자재'+'타부서 구매품') )
		group by 구분,도우모델
		union all 
