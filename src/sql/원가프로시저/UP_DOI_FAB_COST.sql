...
BEGIN
   ...
   where  a.YYYYMM = @YYYYMM AND a.SITE = @SITE AND a.sel_code = @SEL_CODE
	 ...
	 values
	 (@YYYYMM, @SEL_CODE, @SITE, getdate(), @Message, 'system', '가공비 배부', 'UP_DOI_FAB_COST', 'SUCCESS');
 ...
 END TRY
   ...
   BEGIN CATCH
       ...
       INSERT INTO doi_execlog
       	 values
	 	(@YYYYMM, @SEL_CODE, @SITE, getdate(), @Message, 'system', '가공비 배부', 'UP_DOI_FAB_COST', 'FAIL');	
       SELECT @Message as retMessage;
       RETURN -1;
