CREATE PROCEDURE DOI_TotalCost_Tee
(
    @YYYYMM VARCHAR(6),
    @SITE   VARCHAR(4),
    @SELCODE VARCHAR(6)
...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @SumYangsan_Margin NVARCHAR(MAX)=N'0';
		DECLARE @SumDev_Margin     NVARCHAR(MAX)=N'0';
		DECLARE @SumCas_Margin     NVARCHAR(MAX)=N'0';
		DECLARE @SumPur_Margin     NVARCHAR(MAX)=N'0';

        DECLARE @SCOFTotal DECIMAL(18,2) = 0;
        DECLARE @FixTotal  NVARCHAR(MAX)=N'0';
 ...
 SALES_BASE AS (
       ...
       SELECT @FixTotal = CAST(COALESCE(SUM(AMT),0) AS DECIMAL(18,2)) 
       FROM #FIX
        ...
        FROM (
            ...
            SELECT N'16' tree_id,     47 AS rn,        N'  VI. 판관비'        UNION ALL
            SELECT N'17' tree_id,     48 AS rn,        N'  VII. 총원가'       UNION ALL
            SELECT N'18' tree_id,     49 AS rn,        N'  VIII. 영업이익'    UNION ALL
            SELECT N'18.01' tree_id,  50 AS rn,        N'    영업이익률'  UNION ALL
            SELECT N'19' tree_id,     51 AS rn,        N'  IX. 한계이익'      UNION ALL
            SELECT N'19.01' tree_id,  52 AS rn,        N'    한계이익률'    UNION ALL
            SELECT N'20' tree_id,     53 AS rn,        N'  X. 손익분기점'+ REPLICATE(NCHAR(0x3000), 5)
        ) A;
...
--            UNION ALL SELECT rn, gubun, 구분, model, amt FROM MERCH_COGS    --26/02/13 KYH삭제         
        ...
        -------------
		SELECT @SumYangsan_Margin =
		  COALESCE(STRING_AGG(N'COALESCE(Margin.' + QUOTENAME(pivot_key) + N',0)', N' + ')
		    WITHIN GROUP (ORDER BY sort_structure, model), N'0')
		FROM #MODEL
		WHERE 구분=N'양산' AND is_cassette=0;

       	SELECT @SumDev_Margin =
		  COALESCE(STRING_AGG(N'COALESCE(margin.' + QUOTENAME(pivot_key) + N',0)', N' + ')
		    WITHIN GROUP (ORDER BY sort_structure, model), N'0')
		FROM #MODEL
		WHERE 구분=N'개발' AND is_cassette=0;
		...
		SELECT
		    ...
		    , CAST(
					CASE
					  ...
					  WHEN Cur.rn = 52 THEN
						  ('+@FixTotal + ')
					      /NULLIF(((' + @SumYangsan_Margin + ')+(' + @SumDev_Margin + ')+(' + @SumCas_Margin + ')+(' + @SumPur_Margin +')),0)*100
		...
		LEFT JOIN P Margin   ON Margin.rn   = 53
