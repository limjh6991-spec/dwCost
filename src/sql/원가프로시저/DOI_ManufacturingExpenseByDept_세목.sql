CREATE               Procedure DOI_ManufacturingExpenseByDept_세목(
	@YYYYMM varchar(10),
 	@SITE varchar(4)
 )
AS
...
with sourceTable as (
	...
	left join (
		select 1 rn, '  I. 재료비' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and 
		(
		...
		union all 
		select 1 rn, '  I. 재료비' gubun,'카세트팀' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and 
		(
		...
		union all 
		select 2 rn, '    (1) 원재료_원장' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and mat_gubun='제품' and mat_class='원자재'
		union all 
		select 3 rn, '    (2) 원재료_카세트 부품' gubun,'카세트팀' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and mat_gubun='카세트제품' and mat_class='원자재'
		union all 
		select 4 rn, '    (3) 부재료_필름' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and 자재대분류='필름'
		union all 
		select 5 rn, '    (4) 부재료_트레이' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and 자재대분류='트레이'
		union all 
		select 6 rn, '    (5) 부재료_약액' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and mat_class='약액'
		union all
		select 7 rn, '    (6) 부재료_기타' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
		and MAT_GUBUN+mat_class+coalesce(자재대분류,'1') IN ('제품'+'부자재'+'1','제품'+'부자재'+'부자재','제품'+'더미글라스'+'부자재')
		union all
		select 7 rn, '    (6) 부재료_기타' gubun,'카세트팀' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE
