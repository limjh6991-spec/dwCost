...
CREATE           Procedure DOI_DsitributeMatCostByModel_OLD(
	@YYYYMM varchar(10),
 	@SITE varchar(4),
 	@SEL_CODE varchar(10)
 )
...
BEGIN
	BEGIN TRY
		...
		FROM (SELECT DISTINCT TOP 500 도우모델 from (
			  ...
			  ORDER BY 1 )AS models ;
			select @Null_Columns=  '자재분류,자재대분류,자재중분류,자재명,자재번호,[size],'+ replace('coalesce(['+@Null_Columns+']',',coalesce([]',''); --select @Null_Columns;
		-- 동적 SQL 생성
		SET @SQL = '			
			WITH MAT_INFO AS 
			( 
				SELECT * 
				FROM 
				(	select DISTINCT MAT_CODE,MAT_DESC,[SIZE],MAT_CLASS 
				...
				(	select DISTINCT 자재명,자재번호,자재자산분류,자재대분류,자재중분류 
					from doi_bom_mast a 
					where a.YYYYMM = '''+@YYYYMM + ''' and site='''+@SITE + '''
				)as BOM ON (mat.MAT_CODE = bom.자재번호)
			)
		SELECT 
		    '+ @Null_Columns +'
		FROM
		(select	IIF(a.mat_class = ''원자재'', ''원자재'', ''부자재'') as 자재분류 ,coalesce(b.자재대분류,a.mat_class) as 자재대분류,b.자재중분류,b.자재명,
			...
			union all
			select 자재분류, 자재대분류, 자재중분류, 자재명, 자재번호, size, ''Z합계'' 도우모델, sum(배부금액) 배부금액 from (
				select	IIF(a.mat_class = ''원자재'', ''원자재'', ''부자재'') as 자재분류 ,coalesce(b.자재대분류,a.mat_class) as 자재대분류,b.자재중분류,b.자재명,
						a.자재번호,b.size,
						a.도우모델,a.배부금액
						--a.*,b.*
			...
			group by 자재분류, 자재대분류, 자재중분류, 자재명, 자재번호, size
		) AS SourceTable
		PIVOT 
		(
		    SUM(배부금액)
