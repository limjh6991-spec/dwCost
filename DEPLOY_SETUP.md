# DEPLOY_SETUP — dwisCOST 빌드/배포

개발/운영 모두 **수동 배치 스크립트**로 빌드·배포한다.
서비스 종료와 기동은 수동으로 처리한다.

> ⚠️ 배치 파일은 반드시 **CMD**에서 실행할 것 (PowerShell에서는 오류 발생)
...
- GitHub: `https://github.com/limjh6991-spec/dwCost.git`
...
```cmd
REM 1. CMD 창에서 빌드/배포 실행
C:\DCIS\build\deploy-dev.bat

REM 2. 기존 서비스 CMD 창 수동 종료
...
```cmd
REM 1. CMD 창에서 빌드/배포 실행 (DEPLOY 입력 필요)
C:\DCIS\build\deploy-prod.bat
...
2. `mvn -o package -DskipTests` (오프라인 빌드, clean 없음)
...
- **CMD에서 실행**: PowerShell에서 bat 파일 실행 시 인코딩/문법 오류 발생
- **배치 파일 업데이트**: git pull 후 서버의 복사본도 갱신 필요
  ```cmd
  copy /Y C:\DCIS\build\dwCost-dev\deploy\deploy-dev.bat C:\DCIS\build\
  copy /Y C:\DCIS\build\dwCost-dev\deploy\deploy-prod.bat C:\DCIS\build\
  ```
- **빌드 오류 시**: target 폴더가 잠겨있을 수 있음 → 서비스 종료 후 재시도
