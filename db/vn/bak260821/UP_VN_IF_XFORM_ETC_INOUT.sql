...
-- =============================================================================
CREATE   PROCEDURE UP_VN_IF_XFORM_ETC_INOUT
    @yyyymm  VARCHAR(6),
...
BEGIN
    ...
    INSERT INTO doi_vn_etc_inout
        ([일자], [입출고구분], [원천구분], [기타입출고구분], [품목자산분류], [대분류], [중분류], [소분류], [품명], [품번], [규격], [단위], [단수보정구분], [수량], [금액], [단가], [계정과목], [창고], [사용부서], [거래처], [특이사항], [품목특이사항], [yyyymm], [edit_user], [edit_date])
    SELECT
        s.InOutDate,  -- 일자
        s.InOutName,  -- 입출고구분
