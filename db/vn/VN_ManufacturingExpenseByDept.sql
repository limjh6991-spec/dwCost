	...
	BEGIN TRY
		...
		FROM (
			...
			SELECT LTRIM(RTRIM(d.코스트센터)) dept_name,
				...
				CASE WHEN LEFT(d.계정코드,4)='6272' THEN N'부재료비 (6272)' WHEN d.계정과목=N'공구 및 도구비용 - 상각비용' THEN N'제)공구 및 도구 비용 - 상각비용' WHEN d.계정과목=N'공구 및 도구비용 - 일회성비용' THEN N'제)공구 및 도구 비용 -일회성비용' ELSE REPLACE(a.상위계정과목,N'(간접)',N'') END item,
			...
			WHERE d.yyyymm=@YYYYMM AND d.site=@SITE AND d.비용구분='제조'
			  ...
			  AND LEFT(d.계정코드,3) IN ('622','627') AND LEFT(d.계정코드,4)<>'6272'
			...
			GROUP BY LTRIM(RTRIM(d.코스트센터)),
				...
				CASE WHEN LEFT(d.계정코드,4)='6272' THEN N'부재료비 (6272)' WHEN d.계정과목=N'공구 및 도구비용 - 상각비용' THEN N'제)공구 및 도구 비용 - 상각비용' WHEN d.계정과목=N'공구 및 도구비용 - 일회성비용' THEN N'제)공구 및 도구 비용 -일회성비용' ELSE REPLACE(a.상위계정과목,N'(간접)',N'') END
