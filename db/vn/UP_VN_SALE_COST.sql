CREATE   PROCEDURE UP_VN_SALE_COST
    @YYYYMM VARCHAR(10),
    @SITE VARCHAR(4),
    @SEL_CODE VARCHAR(10)
AS
...
SET @Message = @Message + CHAR(10) + ' [INFO]  ' + CONVERT(VARCHAR(19), GETDATE(), 120) + CHAR(9) 
		...
		insert into DOI_SLCO (YYYYMM, SITE, SEL_CODE, 구분, MODEL, Local구분, 판매단위, 거래처, EXPEN_SEL, EXPEN_SEL명, OUT_qty, OUT_AMT)
		...
		FROM (
		    ...
		    FROM (
		        SELECT
		            ...
		            CASE 
		                WHEN ISNULL(t.TOT_SALE_AMT, 0) = 0 THEN 0
		                ELSE ROUND(e.TOT_ACCT * (CAST(s.SALE_AMT AS FLOAT) / t.TOT_SALE_AMT), 2)
