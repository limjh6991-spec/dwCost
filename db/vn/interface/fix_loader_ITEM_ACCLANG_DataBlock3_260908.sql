/* =====================================================================
 * [VN 인터페이스] 로더 응답블록 정정: UP_VN_IF_LOAD_ITEM / UP_VN_IF_LOAD_ACCLANG
 *   품목(ITEM)·언어별계정항목(ACCLANG) 응답 메인행은 $.DataBlock3 인데
 *   DB의 두 로더가 $.DataBlock1 을 읽어 0건 적재되던 것을 정정.
 *   (WH_STOCK_SUM/BIZ_STOCK_SUM 로더는 DB에 이미 DataBlock3로 정상 — 미포함)
 *   실행: DWCMSTEST (SSMS)  — 각 프로시저는 CREATE OR ALTER(멱등)
 * ===================================================================== */
GO
