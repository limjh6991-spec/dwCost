	...
	-- [복원 260729] 20260514 삭제분 복원 (신규DB 첫달 DOI_BOH_AMT 경비/재료비 배부)
      --생산수불이 있는 물량은 기초금액을 경비와 재료비로 금액 비율로 배부한다 
		...
		select	YYYYMM,SEL_CODE,SITE,구분,MODEL,MODEL_TYPE,PRE_EOH_AMT,기초수량,
			case when 기초수량 ='Y' then round((PRE_EOH_AMT * @경비비율), 2) else PRE_EOH_AMT end as 경비기초,
			case when 기초수량 ='Y' then PRE_EOH_AMT-round((PRE_EOH_AMT * @경비비율), 2) else 0 end 재료비기초  --select *
			INTO #bohAmt
		FROM
			doi_boh_amt 
		where 1=1 
