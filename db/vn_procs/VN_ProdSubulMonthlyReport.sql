CREATE OR ALTER PROCEDURE VN_ProdSubulMonthlyReport
(
    @YYYY VARCHAR(4),
    ...
    BEGIN TRY
        ...
        ;WITH BASE AS (
            ...
            FROM (
                ...
                FROM V_DOI_PROD_SUBUL_VN v WITH (NOLOCK)
		...
		select
			...
			0 기타출고_LOT변환,
			0 기타출고_기타,
			CAST(NULL AS nvarchar(50)) 기타출고_반제품_무상,
			CAST(NULL AS nvarchar(50)) 기타출고_반제품_유상,
			0 OUTETC_MONTH,
			CAST(0 AS bigint) BOH_ADJ,
			CAST(0 AS bigint) OUT_ADJ,
			CAST(0 AS bigint) EOH_ADJ,
			CAST(NULL AS nvarchar(50)) PL전,
			CAST(NULL AS nvarchar(50)) PL후,
			CAST(NULL AS nvarchar(50)) 입고전,
			1 rn
		from
			doi_sale
		WHERE
