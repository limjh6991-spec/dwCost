 ...
 *   @YYYYMM 주면 해당월, 없으면 @YYYY 연 전체. 구조 스텁(측정값 0).
 */
    ...
    BEGIN TRY
        ;WITH mt AS (
            SELECT DISTINCT MODEL, MODEL_TYPE FROM DOI_STOCK WITH(NOLOCK)
             WHERE SITE=@SITE AND ( (NULLIF(@YYYYMM,'') IS NOT NULL AND YYYYMM=@YYYYMM) OR (NULLIF(@YYYYMM,'') IS NULL AND SUBSTRING(YYYYMM,1,4)=@YYYY) ) AND MODEL_TYPE IS NOT NULL AND LTRIM(RTRIM(MODEL_TYPE))<>''
            UNION
        ...
        stco AS (
            SELECT DISTINCT MODEL FROM DOI_STCO WITH(NOLOCK)
             WHERE SITE=@SITE AND ( (NULLIF(@YYYYMM,'') IS NOT NULL AND YYYYMM=@YYYYMM) OR (NULLIF(@YYYYMM,'') IS NULL AND SUBSTRING(YYYYMM,1,4)=@YYYY) )
        )
        SELECT DISTINCT
