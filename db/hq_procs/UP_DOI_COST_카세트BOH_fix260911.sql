/* ============================================================================
   ...
   ■ 202608 적용 시 예상   [2026-09-11 정정 — 최초 기재값이 과대였음]
     모델      BOH_QTY IN_QTY EOH_QTY OUT_QTY LOSS_QTY DENOM      BOH         IN        EOH         OUT  LOSS
     VN034P1      350      0       0     350        0   350  24,496,087  4,229,833     0  28,725,920     0
     VN034P2      293     20       0     310        3   310  19,460,992 10,779,994     0  30,240,986     0
     VN034P3      242     20       0     260        2   260   5,855,910  1,371,277     0   7,227,187     0
     ────────────────────────────────────────────────────────────────────────────────────────────
     합계                                                     49,812,989 16,381,104     0  66,194,093     0
     ...
     [정정 내역] 최초에 IN 24,170,590 / OUT 73,983,579 로 적었으나 IN 이 7,789,486 과대였다.
       202608 카세트 비용풀 축소를 반영하지 않은 것이 원인이다.
         · DOI_DEPT_COST 카세트팀(dept 400/448) 계정 24종(202607) → 14종(202608)
         · ACCT_AMT 합 89,893,399 → 77,994,863
         · DOI_CST_RATE.VINA_CST 0.3303 → 0.1980
         · 제)외주가공비 53020010(100% VINA 특례) 202608 소멸
       STEP7 Target 합 = 31,382,112.49(202607) → 15,442,982.87(202608)
       모델별 경비 = 4,229,833 / 10,193,913 / 1,019,237, 재료비 = 0 / 586,081 / 352,040
     [검산] 같은 방법을 202607 에 적용하면 16,506,991 / 12,672,097 / 2,203,024 가 나오고
       실적 DOI_COST 경비 IN 16,507,015 / 12,672,087 / 2,203,011 과 ±24원(반올림)으로 일치한다.

   ■ 현행 코드로 202608 을 돌릴 경우 (대조군)
     VN034P1 DENOM=0(IN_QTY 0·LOSS_QTY 0) → 전량손실 오분류 : OUT 0 / LOSS 4,229,833
     VN034P2 OUT 10,779,994 / VN034P3 OUT 1,371,277
     기초재공 49,812,989 전액 소멸. 제품제조원가 12,151,271
     (정상 66,194,093 대비 54,042,822 과소), LOSS 4,229,833 과대계상.

   ■ BOH 를 상류 MERGE 경유로 읽는 이유 (전월 DOI_COST 직접 읽기와 비교)
     두 MERGE 의 소스가 모두 `DOI_COST WHERE yyyymm=@PREV_MONTH AND eoh<>0` 이므로
     채택안은 이미 '전월 EOH 에서 가져오는' 방식이다. 경유는 우회가 아니라 필수다.
       202607 → 202608 실측: 카세트 BOH 49,812,989 중
         MATCHED(UPDATE)      경비 42행 24,901,581 + 재료 5행 15,048,276 = 39,949,857
         NOT MATCHED(INSERT)  경비 27행  3,989,132 + 재료 1행  5,874,000 =  9,863,132 (19.80%)
       즉 BOH 의 19.80% 는 당월에 대응 행이 아예 생기지 않아 MERGE 의 INSERT 팔로만 들어온다.
       base 에서 전월 DOI_COST 를 직접 LEFT JOIN 하면 붙을 당월 행이 없어 9,863,132 이 소멸한다.
       (당월 행이 없는 이유 — 경비: 카세트팀 계정 9종 소멸, 재료: VN034P1 IN_MONTH=0 으로
        CST_BOM 총소요량 0, DWE14C1002 는 당월 투입이 없어 MAT_CST 의 HAVING 에서 탈락)
     또한 일반(비카세트) 브랜치도 doi_expen_matl.boh / doi_mat_cost.boh_amt 를 읽으므로
     경유 방식이 이 프로시저의 표준이다.
     경유의 유일한 약점은 실행 순서 의존(`set boh=0` 초기화 후 같은 배치에서 MERGE 복원)이며,
     이는 아키텍처 변경이 아니라 적용 후 대사 (a) 로 막는다.
