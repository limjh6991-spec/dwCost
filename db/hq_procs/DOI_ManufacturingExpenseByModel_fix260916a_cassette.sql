/* ============================================================
   DOI_ManufacturingExpenseByModel fix260916a — 제조경비집계표(제품별) 카세트 대분류 신설
   근거: 「KR 로직 문의 (2).xlsx」 확인요청 6번 / 시트 '6.제조경비집계표'
         변경 전 : 합계(양산/개발) + 양산열(…VN034P1~P3) + 개발열
         변경 후 : 합계(양산/개발/카세트) + 양산열 + 개발열 + 카세트열(VN034P1~P3)
   방식: 리포트 전용 재분류. PIVOT 키 구분 = LEFT(model,2)='VN' → N'카세트'.
         조인/집계(Y)는 원본 구분 그대로라 금액 총액 불변, 양산합계에서 카세트만큼 빠진다.
   주의: 화면 컬럼 메타는 매퍼 C0009005_Tab090005_Col 이 따로 만든다. 같이 배포해야 열이 뜬다(jar 재빌드).
   기준: db/hq_procs/DOI_ManufacturingExpenseByModel_fix260914.sql (2026-09-16 운영 정의와 동일 확인)
   검증: 2026-09-16 애드혹 읽기전용 실행(202608 HQ ACTUAL) — 엑셀 '변경 후' 와 원 단위 일치.
         양산VN034P1~P3 열 → 카세트VN034P1~P3 로 이동, Z카세트합계 신설(총 64→65열).
         재료비   합계 661,875,436 불변 / 양산 594,288,603 → 593,350,482 / 카세트 938,121
         노무비   합계 1,720,557,656 불변 / 양산 1,504,313,156 → 1,491,516,648 / 카세트 12,796,508
         경비     합계 1,432,416,299 불변 / 양산 1,252,358,665 → 1,241,922,704 / 카세트 10,435,961
         당기총제조원가 카세트 24,170,590 (P1 6,363,362 / P2 15,921,856 / P3 1,885,372)
   ============================================================ */
-- [2026-09-16a] 카세트 대분류: #GB_CAS(합계열 구분 목록) + 모델열/#sourceTable 구분 재매핑.
...
BEGIN
	BEGIN TRY
		...
		/* [2026-09-16a] 카세트 대분류 신설 — 합계열(Z구분합계)용 구분 목록.
		   원본 구분(양산/개발)에 더해 VN* 모델을 N'카세트' 로 재분류한 목록을 만든다. */
		DROP TABLE IF EXISTS #GB_CAS;
		SELECT DISTINCT CASE WHEN LEFT(model,2) = N'VN' THEN N'카세트' ELSE 구분 END AS 구분
		INTO #GB_CAS
		FROM (
		...
		FROM (SELECT TOP 500 CONCAT(구분, model) model 
				FROM (
					SELECT DISTINCT CASE WHEN LEFT(model,2) = N'VN' THEN N'카세트' ELSE 구분 END AS 구분, model   /* [2026-09-16a] 카세트 대분류 */
					FROM (
						SELECT DISTINCT 구분, replace(도우모델,' ','') as model 
						FROM doi_mat_cost 
						where yyyymm=@YYYYMM 
						...
						SELECT 'Z' 구분, CONCAT(구분, '합계') model 
						  FROM #GB_CAS a   /* [2026-09-16a] 카세트 포함 구분 목록 */
						UNION
...
with sourceTable as (
	select CASE WHEN LEFT(X.model,2) = N'VN' THEN N'카세트' ELSE X.구분 END AS 구분   /* [2026-09-16a] 카세트 대분류 */
	     , X.model, X.rn, X.gubun, COALESCE(Y.amt,0) amt from (
		select * from (
