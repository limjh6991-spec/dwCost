/* ============================================================
   DOI_ProductCOGS fix260915k — 매출원가(제품) 출고(OUT) = 양품(출고) + 양품(반품입고) + 타계정(기타출고 합)
   기준: 운영 2026-09-15 17:13:05 (fix260915i). 리포트 전용 SELECT — 결산 테이블·매출원가 불변, 재결산 불필요.
   ============================================================ */
IF (SELECT CONVERT(varchar(19), modify_date, 120) FROM sys.procedures WHERE name = 'DOI_ProductCOGS') <> '2026-09-15 17:13:05'
BEGIN
    RAISERROR(N'DOI_ProductCOGS 가 기준 버전(2026-09-15 17:13:05)이 아닙니다. 운영 최신 정의로 다시 만드세요.', 16, 1);
    SET NOEXEC ON;
END
GO
