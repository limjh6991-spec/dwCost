...
BEGIN
	  ...
	  select  YYYYMM,
	   ...
	   from (      		 
	  	select
	         ...
	         round(case when boh_qty+in_qty = out_qty+loss_qty and out_qty != 0 then boh + "in"
	         		 else coalesce((boh + "in"-(unit_cost * (eoh_qty / 2.0)))/ nullif(out_qty+bad_qty+Transfer_qty, 0), 0) * out_qty end,0) as out, --out금액 : out_단가 * out수량
	         --round(case when boh_qty+in_qty = loss_qty then 0 else boh+"in"-(unit_cost * (eoh_qty / 2)) end,0) as out1,
	         round(case when boh_qty+in_qty = loss_qty then boh+"in" else 0 end,0) as loss,
	         round(case when boh_qty+in_qty = bad_qty+loss_qty and bad_qty != 0 then boh + "in"
	         		 else coalesce((boh + "in"-(unit_cost * (eoh_qty / 2.0)))/ nullif(out_qty+bad_qty+Transfer_qty, 0), 0) * bad_qty end,0) as bad, --Bad금액 : out_단가 * Bad수량(0)
	         round(case when boh_qty+in_qty = Transfer_qty+loss_qty and Transfer_qty != 0 then boh + "in"
	         		 else coalesce((boh + "in"-(unit_cost * (eoh_qty / 2.0)))/ nullif(out_qty+bad_qty+Transfer_qty, 0), 0) * Transfer_qty end,0) as transfer,*/ --transfer금액 : out_단가 * transfer수량(0)
	         adj_yn,UnitCost_YN
	         --into  DOI_COST
	         /*eoh금액확정
	         eoh확정
   ...
   where  a.YYYYMM = @YYYYMM AND a.SITE = @SITE AND a.sel_code = @SEL_CODE
     ...
     SELECT
      ...
      SET  @Message =  @Message + char(10) + '  [END]' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9)+'- 가공비배부(DOI_COST) 테이블에 '+@YYYYMM + '월 '
	 ...
	 values
	 (@YYYYMM, @SEL_CODE, @SITE, getdate(), ISNULL(@Message, '[로그 생성 경고]'), 'system', '가공비 배부', 'UP_DOI_FAB_COST', 'SUCCESS');
 ...
 END TRY
   ...
   BEGIN CATCH
       ...
       INSERT INTO doi_execlog
       	 (yyyymm,sel_code,site,exec_date,rslt_message,exec_user,menu_id,proc_name,exec_rslt)
       	 values
	 	(@YYYYMM, @SEL_CODE, @SITE, getdate(), ISNULL(@Message, '[로그 생성 경고]'), 'system', '가공비 배부', 'UP_DOI_FAB_COST', 'FAIL');	
       SELECT @Message as retMessage;
       RETURN -1;
