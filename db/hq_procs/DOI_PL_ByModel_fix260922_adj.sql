-- [2026-09-22] DOI_PL_ByModel (3)제품매출원가조정 스펙정렬(총원가&손익 로직 엑셀):
--   (3) = 전량LOSS(모델별, DOI_COST OUT_QTY=0&EOH_QTY=0&LOSS<>0, 부호그대로) + 회계-조정(@CostAdj, 회계열)
--   제품폐기(#DISPOSE)·원부재료폐기(@SCRAP_ADJ)는 (3)에서 제외. (4)재고평가=조정금액 유지.
--   총합계=대분류합=모델합 유지. 리포트 SELECT 전용 → ALTER만.
-- [2026-09-17] DOI_PL_ByModel: 매출원가 하위 '(4) 재고금액평가손실' 신설(DOI_재고자산평가.조정금액 품번별 합산).

-- II.매출원가 가산 + III.매출총이익·V.영업이익 차감(담당자 확정). #INVEVAL을 #MODEL에 편입해 손실전용 모델도 노출.

-- 리포트 전용(SELECT) → ALTER만, 재결산 불필요.

/* ============================================================

   DOI_PL_ByModel fix260916b — 매출↔재고평가 매칭을 품명 → 품번 기준으로 (카세트 매출원가 집계)

   근거: 「KR 로직 문의 (2).xlsx」 확인요청 6번 11행 + 담당자 지시(2026-09-16)

         "매출과 재고평가 금액간 매칭을 재고평가의 모델과 매출의 품명으로 하고 있는데

          매출의 품번으로 매칭으로 변경"

   증상: 카세트 열(VINA N_OPPO 단면바/양면바/하단틀)에 매출액만 있고 II.매출원가가 전부 0.

   원인: 매출 품명이 모델코드가 아니라 한글 품목명이라 재고평가 MODEL(VN034P1~P3)과 조인 실패.

         (양산에 잘못 들어간 게 아니라 리포트에서 통째로 누락 — 총합계 매출원가가 그만큼 는다.)

   방식: #SALEKEY 로 매출 품번 → 모델키를 만들고, 원가 쪽은 구분만 VN→카세트 로 맞춘다.

         리포트 SELECT 전용, 재결산 불필요.

   기준: db/hq_procs/DOI_PL_ByModel.sql (2026-09-16 운영 정의와 동일 확인)

   검증(2026-09-16, 202608 HQ ACTUAL, 애드혹 읽기전용 실행):

         열 60개 그대로, 카세트 열 머리글만 품명 → 품번(카세트VN034P1~P3).

         공통 열 중 바뀐 셀 8개가 전부 카세트합계·Z합계 — 양산·개발 값 변화 0건.

         카세트 매출원가 79,019,006 (P1 30,154,089 / P2 38,703,837 / P3 10,161,080)

         Z합계 매출원가 3,857,660,720 → 3,936,679,726, 영업이익 409,602,133 → 330,583,127

         818VT 는 품번=MODEL 이라 절삭되지 않고 818V 와 분리 유지됨(규칙 ① 이 막아준다).
    ...
    BEGIN TRY
		...
		, COGS_ADJ AS (
		    ...
		    SELECT
		        ...
		        CASE WHEN LEFT(a.model,2)=N'VN' THEN N'카세트' ELSE a.구분 END AS 구분

		        , a.model

		        , SUM(ISNULL(a.LOSS,0)) AS adj_amt
		    ...
		    WHERE a.YYYYMM = @YYYYMM
		      ...
		      AND ISNULL(a.OUT_QTY,0)=0 AND ISNULL(a.EOH_QTY,0)=0 AND ISNULL(a.LOSS_QTY,0)<>0  -- 전량 LOSS(재공 전량손실)
