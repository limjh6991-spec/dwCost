-- [2026-09-17] DOI_PL_ByModel: 매출원가 하위 '(4) 재고금액평가손실' 신설(DOI_재고자산평가.차이 품번별 합산).
...
BEGIN
    ...
    BEGIN TRY
		...
		----------------------------------------------------------------------
		-- [재고평가손실] DOI_재고자산평가 품번→모델키 매핑 후 구분별 차이 집계 (#INVEVAL)
		--   재공품/제품 무관 SUM(차이), 동일품번 양쪽이면 합산. 매핑=#STCO_MODEL 규칙(#SALEKEY 동일).
		----------------------------------------------------------------------
		...
		SELECT 구분, model, CAST(SUM(차이) AS DECIMAL(18,2)) AS amt
		INTO #INVEVAL
		FROM EVAL_MAP GROUP BY 구분, model HAVING SUM(차이) <> 0;
