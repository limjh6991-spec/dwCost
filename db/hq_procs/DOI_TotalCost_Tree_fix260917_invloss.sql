-- [2026-09-17] DOI_TotalCost_Tree: 매출원가 하위 '(4) 재고금액평가손실'(14.04, rn47.5) 신설(DOI_재고자산평가.차이).
...
--	   FROM DOI_SCOF WITH(NOLOCK)
    ...
    , @ACC_ADJ = -@SCOF_ACC   /* 41002020 삭제 → 회계-조정 매출액 = 제품매출-유상사급 = -유상사급 2026-07-31 */
		...
		-- [재고평가손실] DOI_재고자산평가.차이 (품번 SUM → 재공품/제품 동일품번 자동합산)
		SELECT
		    @EvalLoss         = COALESCE(SUM(차이),0),
		    @EvalLossYangsan  = COALESCE(SUM(CASE WHEN 구분=N'양산' AND LEFT(품번,2)<>N'VN' THEN 차이 END),0),
		    @EvalLossDev      = COALESCE(SUM(CASE WHEN 구분=N'개발' AND LEFT(품번,2)<>N'VN' THEN 차이 END),0),
		    @EvalLossCassette = COALESCE(SUM(CASE WHEN LEFT(품번,2)=N'VN' THEN 차이 END),0)
