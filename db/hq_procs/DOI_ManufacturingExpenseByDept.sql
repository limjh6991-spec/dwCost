...
/*		union all select 90 rn, '      1. 제)견본비' gubun*/
	...
	left join (
		/* [변경 전] 2026-07-13 원가자재분류 기반 조건 누락으로 I.재료비 합계 불일치
		...
		[변경 전 끝] */
		-- [변경] 2026-07-13 원가자재분류 IN 조건 추가
		-- [사유] 자재대분류='부자재'이지만 원가자재분류='트레이'인 자재가 I.재료비에서 누락
		select 1 rn, '  I. 재료비' gubun,'제조공통' dept_name, sum(in_amt) amt from doi_mat_amt where yyyymm = @YYYYMM and site = @SITE and sel_code = @SEL_CODE
		and 
		(
			(mat_class in ('원자재','더미글라스','약액'))
		...
		union all 
		select 3 rn, '    (2) 원재료_카세트 부품' gubun,'제조공통' dept_name, sum(배부금액) amt from doi_mat_cost where yyyymm = @YYYYMM and site = @SITE and sel_code = @SEL_CODE
