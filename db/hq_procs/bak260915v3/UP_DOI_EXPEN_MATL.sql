-- 운영 정의 백업 2026-09-15 / modify_date 2026-09-15 14:02:09
...
FROM split;
	...
	SELECT
	    ...
	    0 AS bad_qty,
	    0 AS transfer_qty,
		CAST(ISNULL(u.RMA_UNIT_COST,0) AS NUMERIC(24,12)) AS unit_cost,  -- [2026-09-14] RMA 재투입단가=전월 RMA재고 단가(담당자 방식, B안 되돌림)
	    0 AS boh,
	    CAST(ROUND(ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY, 0) AS NUMERIC(15,2)) AS [in],  -- [2026-09-14] 재공 재투입금액=전월RMA재고(전월단가x수량), 당월순증은 매출원가 크레딧으로 분리
	    999 AS disp_seq,
	    ...
	    q.ETC_IN_RMA_QTY,
	    ROUND(ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY, 0) AS ETC_IN_RMA_AMT
