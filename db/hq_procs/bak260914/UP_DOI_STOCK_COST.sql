-- 운영(도우제조원가시스템) 정의 백업 2026-09-14 / modify_date 2026-09-11 14:55:00
...
/*	        Final_OUTETC as OUTETC_AMT,*/
			...
			OUT_OTHER_QTY,
	        0 AS IN_OTHER_AMT,
            coalesce(OUT_RMA_AMT,0) + coalesce(OUT_DISPOSE_AMT,0) + coalesce(OUT_RND_AMT,0) + coalesce(OUT_OTHER_AMT,0) + coalesce(ROUND(OUT_REWORK_QTY * OUT_UNIT_AMT,0),0) AS OUTETC_AMT,
			RMA_IN_AMT,
			OUT_RMA_AMT,
