-- 운영(도우제조원가시스템) 정의 백업 2026-09-14 / modify_date 2026-07-16 19:24:31
...
BEGIN
    ...
    BEGIN TRY
        ---------------------------------------------------------------------
                -- ======================================================================
        -- STEP 0: 시작 로그 + 데이터 체크 (DOI_MODEL_MAST, DOI_COST, DOI_STOCK)
        -- ======================================================================
        -- START LOG
        ---------------------------------------------------------------------
...
---------------------------------------------------------------------
 ...
 + N' 제품BOH 데이타 ' + CAST(@@ROWCOUNT AS VARCHAR) + N'건을 삭제 했습니다';
        ...
        ---------------------------------------------------------------------
                -- ======================================================================
        -- STEP 2: #stboh 임시테이블 생성 (OUT단가 × DOI_STOCK 조인)
        -- ======================================================================
...
--        UPDATE doi_stock_boh SET boh_amt = 0 where yyyymm = @YYYYMM 
		...
		(
		    SELECT
		        ...
		        P.구분,
		        P.MODEL,
		        COALESCE(X.MODEL_TYPE, Y.MODEL_TYPE) AS MODEL_TYPE,
		        P.EXPEN_SEL,
		        P.EXPEN_SEL명,
		        P.ACCT_NAME,
		        P.EOH_AMT,
 ...
 /*      
        ---------------------------------------------------------------------
                -- ======================================================================
        -- STEP 3: BOH 총액 부착 (DOI_ST_BOH_AMT → #stboh)
        -- ======================================================================
  ...
  PARTITION BY d.YYYYMM, d.SITE, d.sel_code, d.구분, d.model, d.model_type
        ...
        -- ======================================================================
        -- STEP 4: DOI_STOCK_BOH 최종 INSERT (BOH 배부 + 단수차 보정)
        -- ======================================================================
...
*/
       ...
       ---------------------------------------------------------------------
        ...
        SET @Message = @Message + CHAR(10)
			        ...
			        -- ======================================================================
        -- STEP 6: DOI_STCO RMA 처리 (중복삭제, RMA금액, AREAOUT)
        -- ======================================================================
        -- RMA STCO 중복 삭제      
/*			WITH duplicates AS (
			    SELECT model,
