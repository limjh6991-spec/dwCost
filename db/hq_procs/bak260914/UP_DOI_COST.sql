-- 운영(도우제조원가시스템) 정의 백업 2026-09-14 / modify_date 2026-09-14 16:40:24
...
BEGIN
	...
	END
     ...
     SET  @Message =  @Message + char(10) + ' [INFO] ' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9)+'- 가공비배부(DOI_COST) 테이블에 '+@YYYYMM + '월 '
	  ...
	  (YYYYMM ,sel_code ,SITE ,구분 ,model ,expen_sel명 ,ACCT_NAME ,ITEM_NAME ,EXPEN_SEL ,boh_qty ,in_qty ,eoh_qty ,out_qty ,loss_qty ,bad_qty ,transfer_qty ,adj_qty,unit_cost ,boh ,[in] ,eoh ,out_단가 ,[out] ,loss ,bad ,transfer,adj_yn,UnitCost_YN,out_etc ,LOSS_DEFECT_QTY ,LOSS_DEFECT_AMT ,ETC_IN_RMA_QTY ,ETC_IN_RMA_AMT)
...
--	 			WHEN @SEL_CODE != 'ACTLSS' then 0
	         ...
	         round(case when @SITE='HQ' and boh_qty+in_qty = loss_qty and coalesce(defect_qty,0)<>0
	                    ...
	                    else 0 end,0) as LOSS_DEFECT_AMT, ETC_IN_RMA_QTY, ETC_IN_RMA_AMT
	      ...
	      from (
	         ...
	         (SELECT ISNULL(SUM(p.공정발생불량),0) FROM doi_prod_subul p
	            ...
	            WHERE p.yyyymm=doi_expen_matl.yyyymm AND p.site=doi_expen_matl.site
	              ...
	              AND p.구분=doi_expen_matl.구분 AND p.도우모델=doi_expen_matl.model) AS defect_qty, ETC_IN_RMA_QTY, ETC_IN_RMA_AMT
	     ...
	     boh_qty,
	         ...
	         (SELECT ISNULL(SUM(p.공정발생불량),0) FROM doi_prod_subul p
	            ...
	            WHERE p.yyyymm=doi_mat_cost.yyyymm AND p.site=doi_mat_cost.site
	              ...
	              AND p.구분=doi_mat_cost.구분 AND p.도우모델=doi_mat_cost.도우모델) AS defect_qty, 0 AS ETC_IN_RMA_QTY, 0 AS ETC_IN_RMA_AMT
