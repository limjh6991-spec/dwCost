...
BEGIN
    ...
    BEGIN TRY
        ...
        , PL_HEAD AS (
            ...
            --  I. 매출액
            ------------------------------------------------------------------
            SELECT 1 rn, '  I. 매출액' AS gubun, M.구분, M.model, CASE WHEN M.구분 = N'회계' THEN 0 ELSE ISNULL(S.total_sale_amt,0) - ISNULL(SC.scof_amt,0) END AS amt FROM #MODEL M
...
--		VALUES (N'', N'Z합계', 12, N'    (3) 제품매출원가조정', @CostAdj + @LossAdj);
		...
		UPDATE #sourceTable SET amt = COALESCE(amt,0) - @SCOF_ACC WHERE model = N'Z합계' AND rn = 1;
		UPDATE #sourceTable SET amt = COALESCE(amt,0) - @SCOF_ACC WHERE model = N'Z합계' AND rn = 13;

		UPDATE #sourceTable
		SET amt = COALESCE(amt,0) - @SCOF_ACC
		WHERE model = N'Z합계'
		  AND rn = 141;
