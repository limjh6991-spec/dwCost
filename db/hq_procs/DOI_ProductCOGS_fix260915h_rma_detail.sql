/* ============================================================
   DOI_ProductCOGS fix260915h — 매출원가(제품) 입고/출고상세 RMA(품번변경)·기타 표시 수정 (08월 CMS 결산 문의 #9)
   기준: 운영 2026-09-15 15:49:52 (fix260915g 하이브리드). 리포트 전용 SELECT — 결산 테이블·매출원가 불변, 재결산 불필요.
   ============================================================ */
IF (SELECT CONVERT(varchar(19), modify_date, 120) FROM sys.procedures WHERE name = 'DOI_ProductCOGS') <> '2026-09-15 15:49:52'
BEGIN
    RAISERROR(N'DOI_ProductCOGS 가 기준 버전(2026-09-15 15:49:52)이 아닙니다. 운영 최신 정의로 다시 만드세요.', 16, 1);
    SET NOEXEC ON;
END
GO
...
--            , RI.RMA_IN_AMT
            ...
            [변경 전 끝] */
            -- [변경] 2026-09-15h 양산 반품(품번변경) 행은 출고상세 합(RMA(품번변경) 표시금액 포함)으로 표시. 그 외 행은 기존과 같음
