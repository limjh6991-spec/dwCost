-- [2026-09-17] DOI_TotalCost_Tree: 매출원가 하위 '(4) 재고금액평가손실'(14.04, rn47.5) 신설(DOI_재고자산평가.조정금액).
-- V.매출원가·VII.총원가 가산 + VIII.영업이익 차감(담당자 확정). 합계열=스칼라 @EvalLoss* override. rn DECIMAL화.
-- 리포트 전용(SELECT) → ALTER만, 재결산 불필요.
/* ============================================================
  ...
  FROM #MODEL M
		...
		SELECT
		    ...
		    , CAST(
					...
					WHEN Cur.rn = 44 THEN
					    ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ')) + ( @CostAdj /*+ @LossAdj*/ ) + @DispAdj + @EvalLoss
					...
					WHEN Cur.rn = 77 THEN
					  ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ')) + ( @CostAdj ) + @DispAdj + @EvalLoss
					...
					WHEN Cur.rn = 78 THEN
					    ...
					    -*/
					    (
					      ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ') - @SCOF + (@ACC_TOTAL - @ACC_ADJ) - @EvalLoss )
					     -- + ( @CostAdj )
					    )
					ELSE
		    ...
		    , CAST(
		      ...
		      WHEN Cur.rn = 44 THEN
		          ((' + @SumYangsan + ')) + @LossAdjYangsan + @DispAdjYangsan + @EvalLossYangsan
		      ...
		      WHEN Cur.rn = 77 THEN
		   ((' + @SumYangsan + ')) + @DispAdjYangsan + @EvalLossYangsan
			...
			, CAST(
				CASE
			      ...
			      WHEN Cur.rn = 44 THEN
			          ((' + @SumDev + ')) + @LossAdjDev + @DispAdjDev + @EvalLossDev
			...
			, CAST(	
				CASE
				  ...
				  WHEN Cur.rn = 44 THEN ((' + @SumCassette + ')) + @LossAdjCassette + @DispAdjCassette + @EvalLossCassette
