/* ============================================================
   DOI_ProductCOGS fix260916a — 매출원가(제품) 카세트 구분 신설
   근거: 「KR 로직 문의 (2).xlsx」 확인요청 6번 / 시트 '6.매출원가(제품)'
         변경 전 구분='양산'(VN034P1~P3) → 변경 후 구분='카세트'
   방식: 리포트 SELECT 전용 재분류(LEFT(모델,2)='VN'). 결산 테이블 불변, 재결산·jar 재빌드 불필요.
         #MODEL_BASE/#STCO_AGG 조인은 원본 구분 그대로 두고 출력 구분만 재매핑한다.
   기준: db/hq_procs/DOI_ProductCOGS_fix260915k_out_eq_good_etc.sql (2026-09-16 운영 정의와 동일 확인)
         아래 가드가 운영 정의에 fix260915k 마커가 있는지 검사한다(없으면 배포 중단).
   검증: 2026-09-16 애드혹 읽기전용 실행(2026 HQ ACTUAL) — 646행 그대로,
         VN034P1(8월)·P2(7·8월)·P3(7·8월) 5행만 양산→카세트, 나머지 값 변화 0건,
         구분 출현 순서 개발→양산→카세트→RMA.
   주의: 구분 컬럼 타입이 varchar → nvarchar 로 넓어진다(CASE 에 N'카세트' 포함). 화면 영향 없음.
   ============================================================ */
/* 기준 버전 확인 — 운영 정의가 fix260915k 인지 본문 마커로 검사한다.
   (modify_date 는 fix260915k 배포로 이미 바뀌었으므로 타임스탬프 대신 마커를 쓴다) */
IF NOT EXISTS (
    SELECT 1
    FROM sys.sql_modules m
    JOIN sys.procedures  p ON p.object_id = m.object_id
    WHERE p.name = 'DOI_ProductCOGS'
      AND CHARINDEX(N'[2026-09-15k]', m.definition) > 0
)
BEGIN
    RAISERROR(N'DOI_ProductCOGS 운영 정의가 fix260915k 기준이 아닙니다. 운영 최신 정의를 받아 다시 만드세요.', 16, 1);
    SET NOEXEC ON;
END
GO
-- [2026-09-16a] 카세트 구분 신설: 출력 구분 = LEFT(모델,2)='VN' → N'카세트', 그 외 원본 구분 유지.
--   조인/집계는 원본 구분 그대로라 금액 총액 불변. 정렬 개발→양산→카세트→RMA.
...
/*                , SUM(CASE WHEN EXPEN_SEL IN ('MDAX','MIAX') THEN IN_AMT ELSE 0 END) AS IN_MAT_AMT*/
        ...
        ----------------------------------------------------------------------
        SELECT
              CASE WHEN LEFT(M.모델,2) = N'VN' THEN N'카세트' ELSE M.구분 END AS 구분   -- [2026-09-16a] 카세트(VN034P*) 대분류 분리
            , M.모델
            , M.두께
            , M.Inch
            , M.제품구조
...
--            , RI.RMA_IN_AMT
        ...
        ORDER BY CASE WHEN LEFT(M.모델,2) = N'VN' THEN 3 WHEN M.구분 = N'개발' THEN 1 WHEN M.구분 = N'양산' THEN 2 ELSE 4 END   -- [2026-09-16a] 개발→양산→카세트→RMA 순
               , M.구분, M.모델, G.월번호;
