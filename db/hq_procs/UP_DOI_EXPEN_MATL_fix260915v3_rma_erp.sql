-- [2026-09-15 v3] RMA R/W 재공 재투입 = ERP 품목별투입조회 투입금액(총평균). 7073: 8,146,083 (제품 RMA 공정재투입과 동일)
-- 기준: 운영 UP_DOI_EXPEN_MATL 2026-09-15 14:02:09 (카세트 스왑 복구본). 변경은 STEP9 unit_cost/[in]/ETC_IN_RMA_AMT 3줄뿐.
-- 되돌리기: UP_DOI_EXPEN_MATL_bak260915v3 정의로 ALTER
-- 적용 전 확인: 운영 UP_DOI_EXPEN_MATL 가 2026-09-15 14:02:09 버전일 때만 실행 (다르면 이후 배치 건너뜀)
IF (SELECT CONVERT(varchar(19), modify_date, 120) FROM sys.procedures WHERE name = 'UP_DOI_EXPEN_MATL') <> '2026-09-15 14:02:09'
BEGIN
    RAISERROR(N'UP_DOI_EXPEN_MATL 가 기준 버전(2026-09-15 14:02:09)이 아닙니다. 최신 정의 기준으로 다시 만들어야 합니다.', 16, 1);
    SET NOEXEC ON;
END
GO
-- [2026-09-15e] UP_DOI_EXPEN_MATL 카세트 배부 스왑 원복 (7월/3차엑셀 기준).
-- STEP5 소비제품=UTG(0.802), STEP7 VN034P=VINA_CST(0.198) => 카세트제품 23,232,469.
-- 라이브 정의 기반(STEP9 RMA 재투입단가 등 전부 보존). 배포 후 202608 HQ 전체 재결산 필요(손익 변동).
...
BEGIN
 ...
 BEGIN TRANSACTION;
    ...
    -- ======================================================================
    -- ======================================================================
    -- [2026-09-15e] 카세트팀 가공비 배부 원복(스왑 취소): STEP5=UTG(0.802 소비제품), STEP7=VINA_CST(0.198 VN034P).
-- [사유] 3차 배부엑셀/7월 기준 = 카세트 제품 23,232,469(VINA_CST 0.198). 260914 스왑(89,741,011)은 취소.
    --   변경: STEP5(제품 배부) c.UTG->c.VINA_CST(3), STEP7(VN034P 배부) c.VINA_CST->c.UTG(2)+필터.
    --   효과: 카세트팀 자가소비(UTG,0.802)분 가공비 66,508,542원을 소비제품 대신 카세트제품(VN034P)에 귀속.
    --   VN034P 가공비 23,232,469 -> 89,741,011 (담당자 원가배부와 일치). 총액/EXPEN_SEL별 합 불변.
    --   예외 유지: 53020010 외주가공비 100% VN034P, dept448 소모품 100% 제품.
    -- ======================================================================
...
FROM split;
	...
	RMA_MATL AS
	(
	    -- [2026-09-11] 제품 -> 재공품 R/W 재투입 금액 원천 (결산 담당자 자료 = 08월 품목별투입조회)
	    --  품목자산분류='제품' 인 투입행(예: 7073R 258개 / 8,146,083원)의 투입금액을 그대로 쓴다.
	    --  이 금액은 UP_DOI_MAT_AMT 에서 재료비 집계 대상에서 제외되므로 이중계상이 아니다.
	    SELECT
	        m.품번 AS MAT_CODE,
	        SUM(ISNULL(m.투입수량, 0)) AS RW_QTY,
	        SUM(ISNULL(m.투입금액, 0)) AS RW_AMT
	    FROM DOI_MATL_RESC m
	    WHERE m.YYYYMM = @YYYYMM
	      AND m.SITE = @SITE
	      AND m.SEL_CODE = @SEL_CODE
	      AND m.품목자산분류 = N'제품'
	      AND ISNULL(m.투입금액, 0) <> 0
	    GROUP BY m.품번
	),	
	CURR_QTY AS
	(
	...
	SELECT
	    ...
	    0 AS bad_qty,
	    0 AS transfer_qty,
		CAST(COALESCE(r.RW_AMT / NULLIF(r.RW_QTY, 0), u.RMA_UNIT_COST, 0) AS NUMERIC(24,12)) AS unit_cost,  -- [2026-09-15 v3] RMA 재투입단가=ERP 품목별투입조회 투입단가(총평균). 투입행 없을 때만 전월 RMA재고 단가
	    0 AS boh,
	    CAST(CASE WHEN r.RW_AMT IS NULL THEN ROUND(ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY, 0) WHEN r.RW_QTY = q.ETC_IN_RMA_QTY THEN r.RW_AMT ELSE ROUND(r.RW_AMT / NULLIF(r.RW_QTY, 0) * q.ETC_IN_RMA_QTY, 0) END AS NUMERIC(15,2)) AS [in],  -- [2026-09-15 v3] 재공 재투입=ERP 품목별투입조회 투입금액(총평균) = 제품 RMA 공정재투입과 동일
	    999 AS disp_seq,
	    ISNULL(r.RW_AMT, 0) AS in_ori,
	    ISNULL(c.ADJ_YN, 'Y') AS ADJ_YN,
	    1 AS UnitCost_YN,
	    ISNULL(c.OUTETC_QTY, 0) AS outetc_qty,
	    q.ETC_IN_RMA_QTY,
	    CASE WHEN r.RW_AMT IS NULL THEN ROUND(ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY, 0) WHEN r.RW_QTY = q.ETC_IN_RMA_QTY THEN r.RW_AMT ELSE ROUND(r.RW_AMT / NULLIF(r.RW_QTY, 0) * q.ETC_IN_RMA_QTY, 0) END AS ETC_IN_RMA_AMT  -- [2026-09-15 v3] [in]과 동일
	FROM ETC_IN_RMA_QTY q
	LEFT JOIN PREV_RMA_UNIT u
	  ON q.MODEL = u.MODEL
	LEFT JOIN RMA_MATL r
	  ON r.MAT_CODE = q.MODEL + 'R'	  
	...
	SELECT @RMA_IN_AMT = ISNULL(CAST(SUM([in]) AS BIGINT), 0)
	  FROM DOI_EXPEN_MATL
	 WHERE YYYYMM = @YYYYMM AND SITE = @SITE AND SEL_CODE = @SEL_CODE AND EXPEN_SEL = 'RMA1';
    ...
    -- B. VINA 배부 계산 (카세트팀인 경우에만 추가)
              ...
              BEGIN
                  SET  @Message =  @Message + char(10) + REPLICATE(' ', 5) 
                      ...
                      + LEFT(ISNULL(@EXPEN_NAME, '') + ISNULL(REPLICATE(' ', 3-dbo.DOI_ASCII_COUNT(@EXPEN_NAME)),'') + REPLICATE(NCHAR(0x3000), 15), 17)                      + RIGHT(REPLICATE(' ', 15) + FORMAT(@SOURCE_ITEM_AMT, 'N0'), 15)
...
--     SELECT @Message AS before_log_message;                  
	 ...
	 values
	 (@YYYYMM, @SEL_CODE, @SITE, getdate(), ISNULL(@Message, '[로그 생성 경고]'), 'system', '경비 집계', 'UP_DOI_EXPEN_MATL', 'SUCCESS');
 ...
 IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
       ...
       INSERT INTO doi_execlog
       	 (yyyymm,sel_code,site,exec_date,rslt_message,exec_user,menu_id,proc_name,exec_rslt)
       	 values
	 	(@YYYYMM, @SEL_CODE, @SITE, getdate(), ISNULL(@Message, '[로그 생성 경고]'), 'system', '경비 집계', 'UP_DOI_EXPEN_MATL', 'FAIL');	
       SELECT @Message as retMessage;
       RETURN -1;
   END CATCH;
-- ======================================================================
