/* ============================================================
   UP_DOI_SALE_COST fix260915j — 후처리월 매출원가 타계정 이중 차감 수정
   기준: 운영 2026-07-31 10:49:46 (= db/hq_procs/UP_DOI_SALE_COST_bak260911.sql).
   포함: fix260911(okkw1120, 미반영) 트랜잭션 처리 2줄 + 이번 수정.
   반영 후 대상 월 UP_DOI_SALE_COST 재실행 필요(DOI_SLCO·DOI_SMCE_COST 재생성).
   ============================================================ */
IF (SELECT CONVERT(varchar(19), modify_date, 120) FROM sys.procedures WHERE name = 'UP_DOI_SALE_COST') <> '2026-07-31 10:49:46'
BEGIN
    RAISERROR(N'UP_DOI_SALE_COST 가 기준 버전(2026-07-31 10:49:46)이 아닙니다. 운영 최신 정의로 다시 만드세요.', 16, 1);
    SET NOEXEC ON;
END
GO
-- [2026-09-15j] 후처리월(현 UP_DOI_STOCK_COST 결산: 출고에 타계정 이미 제외)은 매출원가에서 타계정을 다시 빼지 않음.
--   1~7월 등 예전 로직 달, 기타출고(재공 전량LOSS) 행은 기존식 유지.
ALTER PROCEDURE UP_DOI_SALE_COST
    @YYYYMM VARCHAR(10),
    @SITE VARCHAR(4),
    @SEL_CODE VARCHAR(10)
AS
BEGIN
    ...
    DECLARE @POSTPROC BIT = 0;   -- [2026-09-15j] 후처리월 여부
    ...
    BEGIN TRY
		...
		IF @CHECK = 1 BEGIN 
			...
			IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;   -- [2026-09-11] BEGIN TRAN(L24) 후 조기 RETURN 시 트랜잭션 누수(266) 방지
			RETURN -1;
		END 
	        -- 1. 기존 SALE 데이터 삭제
        ...
        SET @Message = @Message + CHAR(10) + ' [INFO]  ' + CONVERT(VARCHAR(19), GETDATE(), 120) + CHAR(9) 
		  ...
		  FROM (
			 ...
			 CASE
		        ...
		        ELSE N'개발'
		      END AS 구분, 품명, 품번, Local구분, 판매단위, 거래처,수량, 원화판매금액
			from DOI_SALE_RESC
			where 1=1 
			  and YYYYMM = @YYYYMM
			  and SITE 	 = @SITE
...
SET @Message = @Message + CHAR(10) + ' [INFO]  ' + CONVERT(VARCHAR(19), GETDATE(), 120) + CHAR(9) 
        ...
        -- [2026-09-15j] 후처리월 판정: 현 UP_DOI_STOCK_COST 로 결산한 달은 양품 저장값(OUT_GOOD_*/OUT_GOOD_RTN_*)이 있고
        --   출고(OUT_AMT)에서 타계정이 이미 빠져 있음. 1~7월 등 예전 로직 달은 출고에 타계정이 포함됨.
        IF EXISTS (SELECT 1 FROM DOI_STCO
		    ...
		    FROM (
		        ...
		        FROM (
		            ...
		            SELECT
		                ...
		                /* [변경 전] 2026-09-15j 후처리월은 출고에 타계정이 이미 빠져 있어 타계정이 이중 차감됨(8월 818V 523,993, 0310 146,381)
		                SUM(coalesce(OUT_AMT,0) - coalesce(OUTETC_AMT,0) + coalesce(OUT_RMA_AMT,0)) AS OUT_AMT  -- [RMA 매출원가 합산] 2026-07-31
		                [변경 전 끝] */
		                -- [변경] 2026-09-15j 후처리월의 기타출고(재공 전량LOSS) 외 행은 타계정을 다시 빼지 않음. 그 외는 기존식
		                SUM(coalesce(OUT_AMT,0)
		                    - CASE WHEN @POSTPROC = 1 AND ISNULL(ACCT_NAME, N'') <> N'기타출고' THEN 0 ELSE coalesce(OUTETC_AMT,0) END
		                    + coalesce(OUT_RMA_AMT,0)) AS OUT_AMT  -- [RMA 매출원가 합산] 2026-07-31
		            FROM DOI_STCO
...
SET @Message = @Message + CHAR(10) + '[FINISH] ' + CONVERT(VARCHAR(19), GETDATE(), 120) + CHAR(9) 
    ...
    BEGIN CATCH
       ...
       -- [2026-09-11] 제거: L503 IF @@TRANCOUNT>0 ROLLBACK 로 이미 롤백됨(이중 롤백 3903 방지)
