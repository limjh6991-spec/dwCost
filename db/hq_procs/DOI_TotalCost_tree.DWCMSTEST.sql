...
--		DECLARE @SumPur_Fix      NVARCHAR(MAX)=N'0';
		...
		DECLARE @ACC_PREV_PRICE DECIMAL(18,2) = 0;   -- 이전가격

		DECLARE @ACC_IDLE_COMP  DECIMAL(18,2) = 0;   -- 비가동보상

		DECLARE @ACC_ADJ        DECIMAL(18,2) = 0;   -- 기타
...
--	   FROM DOI_SCOF WITH(NOLOCK)
      ...
      @ACC_PREV_PRICE =
          ...
          COALESCE(SUM(CASE WHEN 계정코드 = N'41004010' AND 코스트센터 LIKE N'재경그룹%'
    ...
    , @ACC_IDLE_COMP =
          ...
          COALESCE(SUM(CASE WHEN 계정코드 = N'41002010' AND 코스트센터 LIKE N'재경그룹%'
    ...
    , @ACC_ADJ =
          ...
          COALESCE(SUM(CASE WHEN 계정코드 = N'41002020' AND 코스트센터 NOT LIKE N'영업그룹%'
...
--            SELECT N'15.01' tree_id, 46 AS rn,       N'    상품매출원가' UNION ALL  */          
			...
			SELECT N'16.01' AS tree_id, 49 AS rn, N'    (1) 판)임원급여' UNION ALL

			SELECT N'16.02' AS tree_id, 50 AS rn, N'    (2) 판)직원급여' UNION ALL

			SELECT N'16.03' AS tree_id, 51 AS rn, N'    (3) 판)상여금' UNION ALL

			SELECT N'16.04' AS tree_id, 52 AS rn, N'    (4) 판)제수당' UNION ALL

			SELECT N'16.05' AS tree_id, 53 AS rn, N'    (5) 판)퇴직급여' UNION ALL

			SELECT N'16.06' AS tree_id, 54 AS rn, N'    (6) 판)복리후생비' UNION ALL

			SELECT N'16.07' AS tree_id, 55 AS rn, N'    (7) 판)여비교통비' UNION ALL

			SELECT N'16.08' AS tree_id, 56 AS rn, N'    (8) 판)접대비' UNION ALL

			SELECT N'16.09' AS tree_id, 57 AS rn, N'    (9) 판)통신비' UNION ALL

			SELECT N'16.10' AS tree_id, 58 AS rn, N'    (10) 판)수도광열비' UNION ALL

			SELECT N'16.11' AS tree_id, 59 AS rn, N'    (11) 판)세금과공과' UNION ALL

			SELECT N'16.12' AS tree_id, 60 AS rn, N'    (12) 판)감가상각비' UNION ALL

			SELECT N'16.13' AS tree_id, 61 AS rn, N'    (13) 판)지급임차료' UNION ALL

			SELECT N'16.14' AS tree_id, 62 AS rn, N'    (14) 판)수선비' UNION ALL

			SELECT N'16.15' AS tree_id, 63 AS rn, N'    (15) 판)보험료' UNION ALL

			SELECT N'16.16' AS tree_id, 64 AS rn, N'    (16) 판)차량유지비' UNION ALL

			SELECT N'16.17' AS tree_id, 65 AS rn, N'    (17) 판)경상연구개발비' UNION ALL

			SELECT N'16.18' AS tree_id, 66 AS rn, N'    (18) 판)운반비' UNION ALL

			SELECT N'16.19' AS tree_id, 67 AS rn, N'    (19) 판)교육훈련비' UNION ALL

			SELECT N'16.20' AS tree_id, 68 AS rn, N'    (20) 판)도서인쇄비' UNION ALL

			SELECT N'16.21' AS tree_id, 69 AS rn, N'    (21) 판)소모품비' UNION ALL

			SELECT N'16.22' AS tree_id, 70 AS rn, N'    (22) 판)지급수수료' UNION ALL

			SELECT N'16.23' AS tree_id, 71 AS rn, N'    (23) 판)광고선전비' UNION ALL

			SELECT N'16.24' AS tree_id, 72 AS rn, N'    (24) 판)무형자산상각비' UNION ALL

			SELECT N'16.25' AS tree_id, 73 AS rn, N'    (25) 판)견본비' UNION ALL

			SELECT N'16.26' AS tree_id, 74 AS rn, N'    (26) 판)사용권자산감가상각비' UNION ALL

			SELECT N'16.27' AS tree_id, 75 AS rn, N'    (27) 판)주식보상비용' UNION ALL

			SELECT N'16.28' AS tree_id, 76 AS rn, N'    (28) 판)해외시장개척비' UNION ALL
