CREATE OR ALTER PROCEDURE UP_DOI_COST
	...
	END
     ...
     SET  @Message =  @Message + char(10) + ' [INFO] ' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9)+'- 가공비배부(DOI_COST) 테이블에 '+@YYYYMM + '월 '
      ...
      ;WITH STUC_UC AS (
         ...
         -- acct_name 통합 출고단가(가장 최근 발생) : 최신월(=DENSE_RANK 1)의 item_name(제품단가 또는 CST/UTG) OUT_UNITCOST 합

         SELECT 구분, model, expen_sel, acct_name, SUM(OUT_UNITCOST) AS out_unitcost

         FROM ( SELECT 구분, model, expen_sel, acct_name, OUT_UNITCOST,

                       DENSE_RANK() OVER (PARTITION BY 구분, model, expen_sel, acct_name ORDER BY YYYYMM DESC) dr

                FROM DOI_STUC

                WHERE SITE=@SITE AND SEL_CODE=@SEL_CODE AND YYYYMM < @YYYYMM AND COALESCE(OUT_UNITCOST,0)<>0 ) x

         WHERE dr = 1

         GROUP BY 구분, model, expen_sel, acct_name

      )

      INSERT INTO DOI_COST

	  (YYYYMM ,sel_code ,SITE ,구분 ,model ,expen_sel명 ,ACCT_NAME ,ITEM_NAME ,EXPEN_SEL ,boh_qty ,in_qty ,eoh_qty ,out_qty ,loss_qty ,bad_qty ,transfer_qty ,adj_qty,unit_cost ,boh ,[in] ,eoh ,out_단가 ,[out] ,loss ,bad ,transfer,adj_yn,UnitCost_YN,out_etc ,LOSS_DEFECT_QTY ,LOSS_DEFECT_AMT)
...
--	 			WHEN @SEL_CODE != 'ACTLSS' then 0
	         			...
	         			else 0 end,0) as loss, --coalesce((boh + coalesce("in",0)- eoh)/ nullif(out_qty+bad_qty+Transfer_qty+ IIF(@SEL_CODE = 'ACTLSS',loss_qty,0), 0), 0) * loss_qty end,0) as loss,
	         ...
	         -- 공정불량(공정발생불량) 수량
	         case when @SITE='HQ' then coalesce(defect_qty,0) else 0 end as LOSS_DEFECT_QTY,

	         /* [변경 전] 2026-07-14 공정발생불량 금액이 LOSS와 다른 로직으로 계산되어 금액 과대 발생
	         case when @SITE='HQ' and coalesce(defect_qty,0)<>0
	              then case when (out_qty+bad_qty+Transfer_qty+loss_qty)<=0
	                        then case when rn_acct=1 then round(coalesce(defect_qty,0)*coalesce(out_unitcost,0),0) else 0 end
	                        else round(case when boh_qty+in_qty = coalesce(defect_qty,0) then boh+"in"
	                              else coalesce((boh + "in"- eoh)/ nullif(out_qty+bad_qty+Transfer_qty+ loss_qty, 0), 0) * coalesce(defect_qty,0) end,0) end
	              else 0 end as LOSS_DEFECT_AMT
	         [변경 전 끝] */
	         -- [변경] 2026-07-14 LOSS와 동일 금액 로직 적용
	         -- [사유] LOSS=공정발생불량+불량판매, 현재 불량판매 없으므로 LOSS=공정발생불량
	         --        공정발생불량 금액도 LOSS와 동일하게 기초+투입 전량이 LOSS인 경우에만 배부
	         round(case when @SITE='HQ' and boh_qty+in_qty = loss_qty and coalesce(defect_qty,0)<>0

	                    then boh+coalesce("in",0)

	                    else 0 end,0) as LOSS_DEFECT_AMT

	   from (

	      select b.*, uc.out_unitcost,

	             ROW_NUMBER() OVER (PARTITION BY b.구분, b.model, b.EXPEN_SEL, b.acct_name ORDER BY b.[in] DESC, b.item_name) AS rn_acct
	      ...
	      from (
	         ...
	         (SELECT ISNULL(SUM(p.공정발생불량),0) FROM doi_prod_subul p

	            WHERE p.yyyymm=doi_expen_matl.yyyymm AND p.site=doi_expen_matl.site

	              AND p.구분=doi_expen_matl.구분 AND p.도우모델=doi_expen_matl.model) AS defect_qty
	    ...
	    union all    
	     ...
	     boh_qty,
	         ...
	         (SELECT ISNULL(SUM(p.공정발생불량),0) FROM doi_prod_subul p
	            ...
	            WHERE p.yyyymm=doi_mat_cost.yyyymm AND p.site=doi_mat_cost.site

	              AND p.구분=doi_mat_cost.구분 AND p.도우모델=doi_mat_cost.도우모델) AS defect_qty
     ...
     INSERT INTO DOI_COST
     	...
     	(YYYYMM,sel_code,SITE,구분,MODEL,EXPEN_SEL명,ACCT_NAME,ITEM_NAME,EXPEN_SEL,
			...
			BOH_QTY ,IN_QTY ,EOH_QTY ,OUT_QTY ,LOSS_QTY ,BAD_QTY ,TRANSFER_QTY ,ADJ_QTY ,UNIT_COST,OUT_단가,[IN],EOH,[OUT],LOSS,ADJ_YN,UnitCost_YN,LOSS_DEFECT_QTY,LOSS_DEFECT_AMT)
		...
		select
				...
				[IN] - ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)=0 THEN 0 ELSE [IN]*1.0/ps.IN_MONTH * ps.OUT_MONTH END, 0)
				      - ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)<>0 AND ISNULL(ps.IN_MONTH,0)=ISNULL(ps.LOSS_MONTH,0) THEN [IN] ELSE 0 END, 0) AS EOH,

				ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)=0 THEN 0 ELSE [IN]*1.0/ps.IN_MONTH * ps.OUT_MONTH END, 0) AS [OUT],

				ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)<>0 AND ISNULL(ps.IN_MONTH,0)=ISNULL(ps.LOSS_MONTH,0) THEN [IN] ELSE 0 END, 0) AS LOSS,

				'Y' as ADJ_YN,

				1 as UmitCost_YN,
				CASE WHEN @SITE='HQ' THEN ISNULL(ps.공정발생불량,0) ELSE 0 END AS LOSS_DEFECT_QTY,
				ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)<>0 AND ISNULL(ps.IN_MONTH,0)=ISNULL(ps.LOSS_MONTH,0) THEN [IN] ELSE 0 END, 0) AS LOSS_DEFECT_AMT
			...
			from doi_vncst_rate a
	         ...
	         select
	            ...
	            m.배부금액 - ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)=0 THEN 0 ELSE m.배부금액*1.0/ps.IN_MONTH * ps.OUT_MONTH END, 0)
	                      - ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)<>0 AND ISNULL(ps.IN_MONTH,0)=ISNULL(ps.LOSS_MONTH,0) THEN m.배부금액 ELSE 0 END, 0) AS EOH,
	            ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)=0 THEN 0 ELSE m.배부금액*1.0/ps.IN_MONTH * ps.OUT_MONTH END, 0) AS [OUT],
	            ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)<>0 AND ISNULL(ps.IN_MONTH,0)=ISNULL(ps.LOSS_MONTH,0) THEN m.배부금액 ELSE 0 END, 0) AS LOSS,
	            'Y' AS ADJ_YN,
	            1 AS UmitCost_YN,
		            CASE WHEN @SITE='HQ' THEN ISNULL(ps.공정발생불량,0) ELSE 0 END AS LOSS_DEFECT_QTY,
		            ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)<>0 AND ISNULL(ps.IN_MONTH,0)=ISNULL(ps.LOSS_MONTH,0) THEN m.배부금액 ELSE 0 END, 0) AS LOSS_DEFECT_AMT
	         from doi_mat_cost m
	              LEFT JOIN DOI_PROD_SUBUL ps
        ...
        SELECT @SOURCE_ITEM_AMT = ISNULL(CAST(SUM([IN]) AS BIGINT), 0)
              ...
              IF ABS(@ITEM_DIFF) > 10 BEGIN
                  ...
                  SET  @Message =  @Message + char(10) + REPLICATE(' ', 5) 
					  ...
					  + LEFT(ISNULL(@EXPEN_NAME, '')+ ISNULL(REPLICATE(' ',3-dbo.DOI_ASCII_COUNT(@EXPEN_NAME)),'') + REPLICATE(NCHAR(0x3000), 15), 17)
      ...
      BEGIN
       ...
       FROM DOI_COST
         ...
         SET  @Message =  @Message + char(10) + REPLICATE(' ', 5) 
          ...
          + LEFT(ISNULL(@EXPEN_NAME_DTL, '') + ISNULL(REPLICATE(' ',3-dbo.DOI_ASCII_COUNT(@EXPEN_NAME_DTL)),'')+ REPLICATE(NCHAR(0x3000), 13), 18)
 ...
 + '- 재공평가(재료비) 데이터 불일치 발생!';
	 ...
	 (@YYYYMM, @SEL_CODE, @SITE, getdate(), ISNULL(@Message,'[로그 생성 경고]'), 'system', '가공비 배부', 'UP_DOI_COST', 'SUCCESS');
   ...
   BEGIN CATCH
       ...
       -- [수정 2026-08] 에러 메시지를 @Message에 먼저 추가 (기존엔 RETURN -1이 먼저라 실제 SQL 에러가 숨겨졌음)
       SET @Message = @Message + char(10) + '[ERROR] ' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9) + ERROR_MESSAGE();

       INSERT INTO doi_execlog
         values
        (@YYYYMM, @SEL_CODE, @SITE, getdate(), ISNULL(@Message,'[로그 생성 경고]'), 'system', '가공비,재료비 배부', 'UP_DOI_COST', 'FAIL');
       SELECT @Message as retMessage;
       RETURN -1;
   END CATCH;
