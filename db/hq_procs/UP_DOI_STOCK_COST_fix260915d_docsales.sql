-- [2026-09-15d] 양품(출고)/양품(반품입고) 매출문서(DOI_SALE_RESC+DOI_INVOICE_RESC) 기반 표시 분해.
-- OUT_AMT/EOH/매출원가 총액 불변. 표시컬럼만 채움(불변식 OUT_GOOD_AMT+OUT_GOOD_RTN_AMT=OUT_AMT).
-- clean 라이브(dedup+블록C) 기반. 배포 후 재결산 시 매출원가 총액 변동 없음(표시 정합만 개선).
...
/*	        Final_OUTETC as OUTETC_AMT,*/
			...
			OUT_GOOD_RTN_QTY,
	        0 AS IN_OTHER_AMT,
            coalesce(OUT_RMA_AMT,0) + coalesce(OUT_DISPOSE_AMT,0) + coalesce(OUT_RND_AMT,0) + coalesce(OUT_OTHER_AMT,0) + coalesce(CASE WHEN OUT_REWORK_QTY <> 0 THEN ROUND(OUT_REWORK_QTY * OUT_UNIT_AMT,0) - BOH_AMT ELSE 0 END,0) AS OUTETC_AMT  /* [2026-09-14] RMA 재투입: 매출원가 크레딧=당월순증(REWORK-전월재고BOH). 전월재고는 재공 재투입(EXPEN_MATL) */,
			RMA_IN_AMT,
			OUT_RMA_AMT,
...
--	    IN_INV_ADJ_QTY, IN_INV_ADJ_AMT,
	...
	SELECT 
	    ...
	    IN_AMT,
	    EOH_AMT,
	    BOH_AMT + IN_AMT + (IN_OTHER_AMT + RMA_IN_AMT) - EOH_AMT - OUT_RMA_AMT - OUT_DISPOSE_AMT - OUT_RND_AMT - OUT_OTHER_AMT - OUT_REWORK_AMT AS OUT_AMT,
...
--	    IN_INV_ADJ_AMT,
	...
	-- ======================================================================
	-- [2026-09-15d] 양품(출고)/양품(반품입고) 매출문서 기반 표시 분해 (담당자 확정)
	--   매출원가 총액(OUT_AMT)/EOH 불변. 표시컬럼만 채움. 블록C(양산 반품크레딧) 직후 실행.
	--   양품출고수량 = 매출문서(DOI_SALE_RESC 국내 + DOI_INVOICE_RESC 수출) 수량>=0 합/도우모델.
	--   양품반품수량 = 매출문서 수량<0 합(음수). 반품크레딧 금액 = DOI_STOCK.RMA_AMT(현행 반품평가) 음수.
	--   불변식: OUT_GOOD_AMT + OUT_GOOD_RTN_AMT = OUT_AMT (행/총계) => 매출원가 총액 보존.
	--   매핑 = DOI_STOCK.MODEL_TYPE(품번) -> MODEL(도우모델). 크레딧행 = 블록C와 동일 rn=1 행.
	-- ======================================================================
	;WITH doc_sales AS (
	    SELECT map.MODEL,
	    ...
	    JOIN (SELECT DISTINCT MODEL_TYPE, MODEL FROM DOI_STOCK
	          WHERE yyyymm=@YYYYMM AND site=@SITE AND sel_code=@SEL_CODE) map ON map.MODEL_TYPE = s.품번
	    GROUP BY map.MODEL
	)
	UPDATE t SET
	    OUT_GOOD_QTY     = d.qout,
	    OUT_GOOD_RTN_QTY = d.qrtn,
	    OUT_GOOD_AMT     = ISNULL(t.OUT_AMT, 0),
	    OUT_GOOD_RTN_AMT = 0
	FROM DOI_STCO t JOIN doc_sales d ON t.MODEL = d.MODEL
	WHERE t.YYYYMM=@YYYYMM AND t.SITE=@SITE AND t.SEL_CODE=@SEL_CODE AND t.구분 <> N'RMA';

	SET @R_Message = @R_Message + char(10) + ' [INFO]  ' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9)
	    + '- 제품수불금액(DOI_STCO) 양품(출고) 매출문서 기반 표시분해 ' + CAST(@@ROWCOUNT AS VARCHAR) + '건';

	-- 반품크레딧: 모델별 RMA_AMT(블록C와 동일 원천)를 크레딧행(rn=1)에 OUT_GOOD_RTN_AMT로 표시.
	--   OUT_GOOD_AMT(양품출고) = OUT_AMT + RMA_AMT (반품크레딧 제거 = 순수 출고원가). OUT_AMT 불변.
	;WITH rma_stock_sum AS (
