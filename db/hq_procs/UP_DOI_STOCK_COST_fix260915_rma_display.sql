...
--	    IN_INV_ADJ_AMT,
	...
	--     공정재투입(OUT_REWORK_AMT) = 전월재고 BOH_AMT (= 6,821,974)
	--     OUTETC_AMT(타계정=상세합) = BOH_AMT (= 6,821,974, 상세=공정재투입만)
	--     OUT_AMT(순출고) = BOH_AMT - REWORK원가 = -1,324,109  => 화면 양품 = OUT_AMT-OUTETC = -8,146,083
	--   ※ RHS는 UPDATE 전 값 사용(REWORK원가 8,146,083). 재결산마다 INSERT 후 1회 적용.
	-- ======================================================================
	UPDATE DOI_STCO SET
	...
	SET @R_Message = @R_Message + char(10) + ' [INFO]  ' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9)
	    + '- 제품수불금액(DOI_STCO) RMA R/W 재투입 표시재분류(공정재투입/OUTETC=BOH) ' + CAST(@@ROWCOUNT AS VARCHAR) + '건을 반영했습니다';
