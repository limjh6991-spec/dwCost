-- [2026-09-15] 재료비 환산량 = (IN + 기타입고_RMA_RW + OUT + LOSS)/2 (가공비 adj_qty 기준 통일).
-- 결산증빙 제품별가공비 vs 원가항목별재료비 환산수량 불일치(issue#2) 해소. 설정 프로시저→배포 후 재결산 필요.
   ...
   BEGIN TRY
	...
	SET  @Message =  @Message + char(10) + ' [INFO]  ' + CONVERT(VARCHAR(19), GETDATE(), 120) + char(9)+'- 재료비배부(DOI_MAT_COST) 테이블에 '+@YYYYMM + '월 '
		...
		select
			...
			/* [변경 전] 2026-07-14 818VT가 818V와 합산됨
			a.도우모델,
			[변경 전 끝] */
			-- [변경] 2026-07-14 818VT 도우코드는 별도 모델로 분리 (BOM에 818VT 제품명 존재)
			CASE WHEN a.도우코드 = '818VT' THEN '818VT' ELSE a.도우모델 END AS 도우모델,
...
--			sum(a.OUTETC_MONTH) outetc_qty,
			...
			sum(a.IN_MONTH + ISNULL(a.기타입고_RMA_RW,0) + a.OUT_MONTH + a.LOSS_MONTH)/ 2.0 as 환산량,  -- [2026-09-15] RMA 재투입 수량 반영(가공비 adj_qty 기준 통일)
		...
		group by a.yyyymm,a.sel_code,a.site,CASE WHEN a.도우코드='818VT' THEN '818VT' ELSE a.도우모델 END,a.구분,a.Adj_YN
		...
		select	(환산량 * 소요량) as 사용량,
		  ...
		  /* [변경 전] 2026-07-14 818VT가 MODEL_MAST에 없어 INNER JOIN 탈락
		  inner join DOI_MODEL_MAST B ON     --모델별 면적정보
	         (a.도우모델 = b.MODEL and b.YYYYMM = @YYYYMM and a.site = b.SITE)
		  [변경 전 끝] */
		  -- [변경] 2026-07-14 818VT→818V로 매핑하여 MODEL_MAST JOIN (면적 정보 취득)
		  inner join DOI_MODEL_MAST B ON     --모델별 면적정보
	         (CASE WHEN a.도우모델='818VT' THEN '818V' ELSE a.도우모델 END = b.MODEL and b.YYYYMM = @YYYYMM and a.site = b.SITE)
...
WHERE 배부금액+Final_BOH != 0
	...
	--전월 EOH금액, 수량을 가져옴 (2026.03.28 By KYH)
		...
		WHEN NOT MATCHED BY TARGET 
			...
			,s.EOH_QTY,COALESCE(s.in_qty,0),COALESCE(s.c_eoh_qty,s.EOH_QTY),COALESCE(s.out_qty,0),COALESCE(s.loss_qty,0),COALESCE(s.bad_qty,0),COALESCE(s.transfer_qty,0),s.EOH,s.배부방식,'X',0,단가);
