CREATE OR ALTER PROCEDURE DOI_ProdSubulMonthlyReport
(
    @YYYY VARCHAR(4),
...
BEGIN
    ...
    BEGIN TRY
        ...
        ;WITH CODE_BASE AS (
            SELECT DISTINCT
				 구분
                , CASE WHEN LEFT(도우모델,2)=N'VN' THEN 도우코드 ELSE 도우모델 END AS 모델
                , MAX(Inch) AS Inch
                , MAX(DW_Site) AS DW_Site
            FROM #BASE
            GROUP BY  구분,CASE WHEN LEFT(도우모델,2)=N'VN' THEN 도우코드 ELSE 도우모델 END
        )
        SELECT *
        INTO #CODE_BASE
        FROM CODE_BASE;
        ...
        ;WITH AGG AS (
            SELECT
                  b.YYYYMM
                , b.SITE
                , MAX(MAX(b.DW_Site)) over (PARTITION BY CASE WHEN LEFT(b.도우모델,2)=N'VN' THEN b.도우코드 ELSE b.도우모델 END) as DW_Site
                , MAX(MAX(b.Inch))  over (PARTITION BY CASE WHEN LEFT(b.도우모델,2)=N'VN' THEN b.도우코드 ELSE b.도우모델 END)AS Inch
                , CASE WHEN LEFT(b.도우모델,2)=N'VN' THEN b.도우코드 ELSE b.도우모델 END AS 모델
				, 구분
...
--                , SUM(ISNULL(b.OUTETC_MONTH, 0))  AS OUTETC
				, SUM(ISNULL(b.공정발생불량, 0)) AS 공정발생불량
				, SUM(ISNULL(b.불량판매, 0)) AS 불량판매
				, SUM(ISNULL(b.타계정입고, 0)) AS 타계정입고
				, SUM(ISNULL(b.기타입고_LOT변환, 0)) AS 기타입고_LOT변환
				, SUM(ISNULL(b.기타입고_RMA_RW, 0)) AS 기타입고_RMA_RW
				, SUM(ISNULL(b.기타입고_전월불량, 0)) AS 기타입고_전월불량
				, SUM(ISNULL(b.기타입고_당월불량, 0)) AS 기타입고_당월불량
				, SUM(ISNULL(b.타계정출고, 0)) AS 타계정출고
				, SUM(ISNULL(b.기타출고_LOT변환, 0)) AS 기타출고_LOT변환
				, SUM(ISNULL(b.기타출고_기타, 0)) AS 기타출고_기타
...
*/
            FROM #BASE b
            GROUP BY b.YYYYMM, b.SITE, b.DW_Site, b.Inch, CASE WHEN LEFT(b.도우모델,2)=N'VN' THEN b.도우코드 ELSE b.도우모델 END, b.구분
        )
        SELECT *
        INTO #AGG
        FROM AGG;
...
--		        , COALESCE(a.OUTETC, 0)  AS OUTETC
				...
				, COALESCE(a.공정발생불량,0) AS 공정발생불량
				, COALESCE(a.불량판매,0) AS 불량판매
				, COALESCE(a.타계정입고,0) AS 타계정입고
				, COALESCE(a.기타입고_LOT변환,0) AS 기타입고_LOT변환
				, COALESCE(a.기타입고_RMA_RW,0) AS 기타입고_RMA_RW
				, COALESCE(a.기타입고_전월불량,0) AS 기타입고_전월불량
				, COALESCE(a.기타입고_당월불량,0) AS 기타입고_당월불량
				, COALESCE(a.타계정출고,0) AS 타계정출고
				, COALESCE(a.기타출고_LOT변환,0) AS 기타출고_LOT변환
				, COALESCE(a.기타출고_기타,0) AS 기타출고_기타		                    
...
--		        , SUM(OUTETC) AS OUT_ETC
				...
				, SUM(공정발생불량) AS 공정발생불량
				, SUM(불량판매) AS 불량판매
				, SUM(타계정입고) AS 타계정입고
				, SUM(기타입고_LOT변환) AS 기타입고_LOT변환
				, SUM(기타입고_RMA_RW) AS 기타입고_RMA_RW
				, SUM(기타입고_전월불량) AS 기타입고_전월불량
				, SUM(기타입고_당월불량) AS 기타입고_당월불량
				, SUM(타계정출고) AS 타계정출고
				, SUM(기타출고_LOT변환) AS 기타출고_LOT변환
				, SUM(기타출고_기타) AS 기타출고_기타		                    
/*		        , SUM(NG) AS NG
		        , SUM(수율제외) AS 수율제외
