-- [2026-09-15] 재공 RMA R/W 재투입금액 = 전월 RMA재고 단가 x 수량 (담당자 확정 6,821,974)
-- v2(2026-09-14 ERP 품목별투입조회 투입금액 8,146,083)에서 되돌림. 라이브 UP_DOI_EXPEN_MATL 기반.
...
FROM split;
	...
	SELECT
	    ...
	    0 AS bad_qty,
	    0 AS transfer_qty,
		CAST(ISNULL(u.RMA_UNIT_COST, 0) AS NUMERIC(24,12)) AS unit_cost, -- [2026-09-15] RMA 재투입단가=전월 RMA재고 단가(담당자 확정)  -- [2026-09-14 v2] RMA 재투입단가=ERP 품목별투입조회 투입단가(총평균). 투입행이 없을 때만 전월 RMA재고 단가
	    0 AS boh,
	    CAST(ROUND(ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY, 0) AS NUMERIC(15,2)) AS [in], -- [2026-09-15] 재공 재투입=전월단가x수량(6,821,974). v2 ERP투입금액(8,146,083) 되돌림  -- [2026-09-14 v2] 재공 재투입금액=ERP 품목별투입조회 투입금액(총평균 = 제품 RMA창고 공정재투입 출고). 전월단가 방식은 ERP 대비 차이가 나서 되돌림
	    999 AS disp_seq,
	    ...
	    q.ETC_IN_RMA_QTY,
	    ROUND(ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY, 0) AS ETC_IN_RMA_AMT -- [2026-09-15] [in]과 동일(전월단가x수량)  -- [2026-09-14 v2] [in]과 같은 금액	    
