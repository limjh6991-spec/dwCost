-- 기준: 운영 UP_DOI_STOCK_COST 2026-09-15 14:35:15. 변경은 (1) OUTETC_AMT 식 1줄 (2) RMA 표시재분류 UPDATE 블록 주석 처리.
-- 되돌리기: UP_DOI_STOCK_COST_bak260915v3 정의로 ALTER
-- 적용 전 확인: 운영 UP_DOI_STOCK_COST 가 2026-09-15 14:35:15 버전일 때만 실행 (다르면 이후 배치 건너뜀)
IF (SELECT CONVERT(varchar(19), modify_date, 120) FROM sys.procedures WHERE name = 'UP_DOI_STOCK_COST') <> '2026-09-15 14:35:15'
BEGIN
    RAISERROR(N'UP_DOI_STOCK_COST 가 기준 버전(2026-09-15 14:35:15)이 아닙니다. 최신 정의 기준으로 다시 만들어야 합니다.', 16, 1);
    SET NOEXEC ON;
END
GO
