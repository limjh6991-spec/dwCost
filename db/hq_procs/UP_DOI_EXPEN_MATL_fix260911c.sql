...
FROM split;
	...
	DECLARE @RMA_IN_AMT BIGINT = 0;   -- [2026-09-11] RMA 재투입 반영금액
	...
	(
	    ...
	    FROM DOI_STCO s
	    WHERE s.YYYYMM = @PREV_MONTH   -- [2026-09-11] '202512' 하드코딩 제거(DOI_STCO 구분='RMA' 는 202601~ 존재)
	      AND s.SITE = @SITE
	...
	SELECT
	    ...
	    0 AS bad_qty,
	    0 AS transfer_qty,
	    CAST(COALESCE(r.RW_AMT / NULLIF(r.RW_QTY, 0), u.RMA_UNIT_COST, 0) AS NUMERIC(24,12)) AS unit_cost,
	    0 AS boh,
	    CAST(CASE WHEN r.RW_AMT IS NULL THEN ISNULL(u.RMA_UNIT_COST, 0) * q.ETC_IN_RMA_QTY WHEN r.RW_QTY = q.ETC_IN_RMA_QTY THEN r.RW_AMT ELSE ROUND(r.RW_AMT / NULLIF(r.RW_QTY, 0) * q.ETC_IN_RMA_QTY, 0) END AS NUMERIC(15,2)) AS [in],
	    999 AS disp_seq,
	...
	LEFT JOIN RMA_MATL r
	  ON r.MAT_CODE = q.MODEL + 'R'      -- [2026-09-11] R/W 재투입 금액 원천 (7073 -> 7073R)
	...
	-- [2026-09-11] STEP9 가 실제로 넣은 RMA 재투입 금액 (STEP10 무결성 검증 양변 일치용)
	...
	SET @Message = @Message + CHAR(10) + CHAR(10)
	    ...
	    + CAST(@RMA_RW_CNT AS VARCHAR) + '건, ' + ISNULL(FORMAT(@RMA_IN_AMT, 'N0'), '0') + '원을 추가했습니다';      	
 ...
 @TARGET_AMT BIGINT = 0,
      ...
      SET @SOURCE_AMT = @FILTERED_AMT + @RMA_IN_AMT/* + @CASSETTE_AMT*/;
