/* ============================================================
   DOI_ProductCOGS fix260915i — 매출원가(제품) 출고상세 양품(출고)에서 타계정 빼기 제거 (후처리월)
   기준: 운영 2026-09-15 16:54:56 (fix260915h). 리포트 전용 SELECT — 결산 테이블·매출원가 불변, 재결산 불필요.
   ============================================================ */
IF (SELECT CONVERT(varchar(19), modify_date, 120) FROM sys.procedures WHERE name = 'DOI_ProductCOGS') <> '2026-09-15 16:54:56'
BEGIN
    RAISERROR(N'DOI_ProductCOGS 가 기준 버전(2026-09-15 16:54:56)이 아닙니다. 운영 최신 정의로 다시 만드세요.', 16, 1);
    SET NOEXEC ON;
END
GO
