...
--	 			WHEN @SEL_CODE != 'ACTLSS' then 0
     ...
     INSERT INTO DOI_COST
		...
		select
				...
				ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)=0 THEN 0 ELSE [IN]*1.0/ps.IN_MONTH * ISNULL(ps.공정발생불량,0) END, 0) AS LOSS_DEFECT_AMT
			...
			from doi_vncst_rate a
	         ...
	         select
	            ...
	            1 AS UmitCost_YN,
		            CASE WHEN @SITE='HQ' THEN ISNULL(ps.공정발생불량,0) ELSE 0 END AS LOSS_DEFECT_QTY,
		            ROUND(CASE WHEN ISNULL(ps.IN_MONTH,0)=0 THEN 0 ELSE m.배부금액*1.0/ps.IN_MONTH * ISNULL(ps.공정발생불량,0) END, 0) AS LOSS_DEFECT_AMT
	         from doi_mat_cost m
	              LEFT JOIN DOI_PROD_SUBUL ps
