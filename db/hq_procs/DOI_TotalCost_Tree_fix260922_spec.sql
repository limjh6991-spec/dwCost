...
--		DECLARE @SumPur_Fix      NVARCHAR(MAX)=N'0';
		...
		DECLARE @EvalLoss          DECIMAL(18,2) = 0;
		DECLARE @EvalLossYangsan   DECIMAL(18,2) = 0;
		DECLARE @EvalLossDev       DECIMAL(18,2) = 0;
		DECLARE @EvalLossCassette  DECIMAL(18,2) = 0;
...
--	   FROM DOI_SCOF WITH(NOLOCK)
    ...
    , @ACC_ADJ = -@SCOF_ACC   /* 41002020 삭제 → 회계-조정 매출액 = 제품매출-유상사급 = -유상사급 2026-07-31 */
		...
		-- [재고평가손실] DOI_재고자산평가.조정금액 (품번 SUM → 재공품/제품 동일품번 자동합산)
		SELECT
		    @EvalLoss         = COALESCE(SUM(조정금액),0),
		    @EvalLossYangsan  = COALESCE(SUM(CASE WHEN 구분=N'양산' AND LEFT(품번,2)<>N'VN' THEN 조정금액 END),0),
		    @EvalLossDev      = COALESCE(SUM(CASE WHEN 구분=N'개발' AND LEFT(품번,2)<>N'VN' THEN 조정금액 END),0),
		    @EvalLossCassette = COALESCE(SUM(CASE WHEN LEFT(품번,2)=N'VN' THEN 조정금액 END),0)
		FROM DOI_재고자산평가 WITH(NOLOCK)
		WHERE YYYYMM=@YYYYMM AND SITE=@SITE AND SEL_CODE=@SELCODE;
  ...
  SELECT  N'11.03' tree_id, 11 AS rn,        N'    (3) 약액'        UNION ALL
			...
			SELECT N'14.04' AS tree_id, 47.5 AS rn, N'    (4) 재고금액평가손실' UNION ALL      
...
--		
  ...
  EXP_AGG AS (
    ...
    SELECT 99 rn, N'    상품매출원가' gubun, M.구분, M.model,
        ...
        /* [2026-09-22 스펙] 공통 소스: (1)제품매출원가=양품출고+양품반품입고, (3)조정=전량LOSS(재공 전량손실) */
        STCO_COGS AS (
            SELECT
        ...
        FULL_LOSS AS (
            SELECT
                  CASE WHEN LEFT(C.model,2) = N'VN' THEN N'카세트' ELSE C.구분 END AS 구분
                , C.model
                , CAST(SUM(COALESCE(C.LOSS,0)) AS DECIMAL(18,2)) AS amt
            FROM DOI_COST C WITH(NOLOCK)
            WHERE C.YYYYMM = @YYYYMM AND C.SITE = @SITE AND C.SEL_CODE = @SELCODE
              AND ISNULL(C.OUT_QTY,0)=0 AND ISNULL(C.EOH_QTY,0)=0 AND ISNULL(C.LOSS_QTY,0)<>0
            GROUP BY CASE WHEN LEFT(C.model,2) = N'VN' THEN N'카세트' ELSE C.구분 END, C.model
            HAVING SUM(COALESCE(C.LOSS,0)) <> 0
        ),
        ...
        TOTAL_MFG AS (
         ...
         PROD_COGS AS (
		    ...
		    FROM #MODEL M
		    LEFT JOIN STCO_COGS PC
		           ON PC.model = M.model AND PC.구분 = M.구분
		),
		MERCH_COGS_FACT AS (
		    -- 상품매출원가
		    SELECT
...
--		),
    /* ====== VI 판관비 ====== */
        ...
        OP_PROFIT AS (
            -- VIII 영업이익 = 매출액(제품-유상사급+상품) - 총원가  /* [2026-09-22 스펙] 유상사급 모델별 차감 → 총합계=대분류합 */
            SELECT 78 rn, N'  VIII. 영업이익' gubun, M.구분, M.model,
                   CAST(COALESCE(SL.total_sale_amt,0) - COALESCE(SC.scof_amt,0) - COALESCE(TC.amt,0) AS DECIMAL(18,2)) AS amt
            FROM #MODEL M
...
--    		UNION ALL SELECT rn, gubun, 구분, model, amt FROM ETC_SALE_BASE    		    		
		    ...
		    -- [2026-09-22 스펙] 제품폐기(DISPOSE_ADJ_BASE)·스칼라 총합계행(PROD_COGS_ADJ)은 (3)에서 제외
...
STRING_AGG(N'COALESCE(Cur.' + QUOTENAME(pivot_key) + N',0)', N' + ')
  ...
  FROM #MODEL M
		...
		SET @SQL = N'
		;WITH P AS (
		    SELECT tree_id, TRY_CONVERT(DECIMAL(9,2), rn) AS rn, gubun, ' + @Columns + N'
		    FROM (
		        SELECT tree_id, TRY_CONVERT(DECIMAL(9,2), rn) AS rn, gubun, pivot_key, amt
		        FROM #BASE
		    ) S
		...
		SELECT
		    ...
		    , CAST(
					...
					WHEN Cur.rn = 44 THEN   /* [2026-09-22] 매출원가 = 모델합((1)+(2)+(3전량LOSS)) + 회계(@CostAdj) + (4)재고평가(@EvalLoss) */
					    ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ')) + @CostAdj + @EvalLoss

					WHEN Cur.rn = 47 THEN   /* [2026-09-22] (3)조정 = 모델합(전량LOSS) + 회계-조정(@CostAdj) */
					    ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ')) + @CostAdj
					WHEN Cur.rn = 47.5 THEN @EvalLoss

					WHEN Cur.rn = 77 THEN   /* [2026-09-22] 총원가 = 모델합(매출원가+판관) + 회계(@CostAdj) + (4)재고평가(@EvalLoss) */
					  ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ')) + @CostAdj + @EvalLoss
					...
					WHEN Cur.rn = 78 THEN
					    ...
					    -*/
					    (   /* [2026-09-22] 영업이익 = 매출액 - 총원가. 유상사급은 모델(OP_PROFIT)에서 차감 → 회계=@ACC_TOTAL(매출액회계) - @CostAdj(총원가회계) - @EvalLoss */
					      ((' + @SumYangsan + ')+(' + @SumDev + ')+(' + @SumCassette + ')+(' + @SumPurchase + ') + @ACC_TOTAL - @CostAdj - @EvalLoss )
					    )
					ELSE
		    ...
		    , CAST(
		      ...
		      WHEN Cur.rn = 44 THEN
		          ((' + @SumYangsan + ')) + @EvalLossYangsan

		      WHEN Cur.rn = 47 THEN
		          ((' + @SumYangsan + '))
		      WHEN Cur.rn = 47.5 THEN @EvalLossYangsan
		   ...
		   ((' + @SumYangsan + ')) + @EvalLossYangsan
			...
			, CAST(
				CASE
			      ...
			      WHEN Cur.rn = 47 THEN
			          ((' + @SumDev + '))
			      WHEN Cur.rn = 47.5 THEN @EvalLossDev
			...
			, CAST(
			    CASE
			        ...
			        -- [2026-09-22 스펙] (3)조정 회계 = 회계-조정(@CostAdj: DOI_DEPT_COST 제품매출원가 대변). 매출원가/총원가 가산, 영업이익 차감
			        WHEN Cur.rn = 78 THEN @ACC_TOTAL - @CostAdj
			        WHEN Cur.rn IN (44,47,77) THEN @CostAdj
			...
			, CAST(
			    CASE
			        ...
			        WHEN Cur.rn IN (44,47,77) THEN @CostAdj   -- [2026-09-22] (3)제품매출원가조정: 회계-조정
			        ELSE 0
			    END
			...
			, CAST(
			    CASE
			        ...
			        -- [2026-09-22] 원부재료 재고폐기(@ACC_SCRAP)는 (3)조정에서 제외 → 이전가격 가산 삭제
			        ELSE 0
			    END
		...
		ORDER BY TRY_CONVERT(DECIMAL(9,2), Cur.rn);
		';
		
--		SELECT @SQL;
		EXEC sp_executesql @SQL, 
		N'@SCOF DECIMAL(18,2), @CostAdj DECIMAL(18,2), @LossAdj DECIMAL(18,2), @LossAdjYangsan DECIMAL(18,2), @LossAdjDev DECIMAL(18,2), @LossAdjCassette DECIMAL(18,2) ,@ACC_TOTAL decimal(18,2), @ACC_PREV_PRICE decimal(18,2), @ACC_IDLE_COMP decimal(18,2), @ACC_ADJ decimal(18,2), @SCOF_ACC decimal(18,2), @ACC_SCRAP decimal(18,2), @DispAdj decimal(18,2), @DispAdjYangsan decimal(18,2), @DispAdjDev decimal(18,2), @DispAdjCassette decimal(18,2), @EvalLoss decimal(18,2), @EvalLossYangsan decimal(18,2), @EvalLossDev decimal(18,2), @EvalLossCassette decimal(18,2)',
		@SCOF = @SCOFTotal,
    	@CostAdj = @CostAdj,
    	@LossAdj = @LossAdj,
		...
		@EvalLoss=@EvalLoss, @EvalLossYangsan=@EvalLossYangsan, @EvalLossDev=@EvalLossDev, @EvalLossCassette=@EvalLossCassette;
