...
## 1. 설계 원칙
1. **기존 「타시스템(C0007000)」 그룹을 확장/개편**한다 — 이미 외부데이터 수신(엑셀 업로드) 목적의 메뉴이므로 성격이 동일.
...
```
■ 인터페이스 수신 (ERP/MES → CMS)      ← 기존 "타시스템(C0007000)" 확장
│
├─ ① 기초정보(마스터)
...
## 7. 확정 필요 항목 (검토 후 v1.0)
- 최상위 그룹명: "인터페이스 수신(ERP/MES)" vs 기존 "타시스템" 유지
- 마스터(①) 위치: 인터페이스 그룹 vs 별도 "기준정보" 그룹
- 수출Claim ↔ 불량반품 대응 관계 최종 확인
- 화면ID 체계(신규 Cxxxxxx 부여 규칙)
