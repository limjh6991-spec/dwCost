CREATE                                               PROCEDURE DOI_TotalCost
(
    @YYYYMM VARCHAR(6),
    @SITE   VARCHAR(4),
    @SELCODE VARCHAR(6)
...
--		DECLARE @SumPur_Fix      NVARCHAR(MAX)=N'0';
       ...
       WHERE 구분=N'개발'
        ...
        FROM (
            -- I. 매출액
            SELECT  1 AS rn, N'  I. 매출액'        AS gubun UNION ALL
            SELECT  2,        N'    (1) 제품매출'    UNION ALL
            SELECT  3,        N'        수량'       UNION ALL
 			SELECT  4,        N'        단가'       UNION ALL
            SELECT  5,        N'    (2) 유상사급'    UNION ALL
            SELECT  6,        N'    (3) 상품매출'    UNION ALL
            SELECT  7,        N'    (4) 기타매출'    UNION ALL

            -- II. 재료비
            SELECT  8,        N'  II. 재료비'        UNION ALL
            SELECT  9,        N'    (1) 원장'        UNION ALL
            SELECT 10,        N'    (2) PF'          UNION ALL
            SELECT 11,        N'    (3) 약액'        UNION ALL
            SELECT 12,        N'    (4) 트레이'      UNION ALL
            SELECT 13,        N'    (5) 더미글라스'  UNION ALL
            SELECT 14,        N'    (6) 기타'        UNION ALL

            -- III~IV
            SELECT 15,        N'  III. 노무비'       UNION ALL            
            SELECT 16,        N'    (1) 제)임원급여'     UNION ALL
            SELECT 17,        N'    (2) 제)직원급여'     UNION ALL
            SELECT 18,        N'    (3) 제)상여금'       UNION ALL
            SELECT 19,        N'    (4) 제)제수당'       UNION ALL
            SELECT 20,        N'    (5) 제)퇴직급여'     UNION ALL
            SELECT 21,        N'    (6) 제)주식보상비용'   UNION ALL
            SELECT 22,        N'  IV. 제조경비'     	 UNION ALL
            SELECT 23,        N'    (1) 제)복리후생비'   UNION ALL
	        SELECT 24,        N'    (2) 제)여비교통비'  UNION ALL
            SELECT 25,        N'    (3) 제)통신비'    UNION ALL
            SELECT 26,        N'    (4) 제)수도광열비'  UNION ALL
            SELECT 27,        N'    (5) 제)전력비'     UNION ALL
            SELECT 28,        N'    (6) 제)세금과공과'   UNION ALL
            SELECT 29,        N'    (7) 제)감가상각비'   UNION ALL
            SELECT 30,        N'    (8) 제)지급임차료'   UNION ALL
            SELECT 31,        N'    (9) 제)수선비'     UNION ALL
            SELECT 32,        N'    (10) 제)보험료'    UNION ALL
            SELECT 33,        N'    (11) 제)차량유지비'  UNION ALL
	        SELECT 34,        N'    (12) 제)운반비'    UNION ALL
            SELECT 35,        N'    (13) 제)교육훈련비'  UNION ALL
            SELECT 36,        N'    (14) 제)도서인쇄비'  UNION ALL
            SELECT 37,        N'    (15) 제)소모품비'   UNION ALL
            SELECT 38,        N'    (16) 제)지급수수료'  UNION ALL
            SELECT 39,        N'    (17) 제)외주가공비'  UNION ALL
            SELECT 40,        N'    (18) 제)사용권자산감가상각비' UNION ALL
            SELECT 41,        N'    (19) 제)검사비' UNION ALL
            SELECT 42,        N'    (20) 제)견본비' UNION ALL

            SELECT 43,        N'  V. 매출원가' UNION ALL
		    /*SELECT 44,        N'    매출원가조정' UNION ALL            

            -- V~X
            SELECT 45,        N'  V. 재고조정'       UNION ALL
            SELECT 46,       N'    상품매출원가' UNION ALL  */          
            SELECT 47,        N'  VI. 판관비'        UNION ALL
            SELECT 48,        N'  VII. 총원가'       UNION ALL
            SELECT 49,        N'  VIII. 영업이익'    UNION ALL
SELECT 50,        N'    영업이익률'  UNION ALL
            SELECT 51,        N'  IX. 한계이익'      UNION ALL
            SELECT 52,        N'    한계이익률'    UNION ALL
            SELECT 53,        N'  X. 손익분기점'+ REPLICATE(NCHAR(0x3000), 5)
        ) A;
       ...
       SALES_FACT AS (
        ...
        TOTAL_MFG AS (
			...
			SELECT 43 rn, N'  V. 매출원가' gubun, M.구분, M.model,
                   CAST(COALESCE(II.amt,0) + COALESCE(III.amt,0) + COALESCE(IV.amt,0) AS DECIMAL(18,2)) AS amt
            FROM #MODEL M
        ...
        */
        ...
        /* ====== 상품매출원가 ====== */
        MERCH_COGS AS ( 
            SELECT 46 rn, N'    상품매출원가' gubun, M.구분, M.model,
    ...
    /* ====== VI 판관비 ====== */
      ...
      SELECT
        ...
        TOTAL_COST AS (
			...
			SELECT 48 rn, N'  VII. 총원가' gubun, M.구분, M.model,
        ...
        OP_PROFIT AS (
            -- VIII 영업이익 = 매출액 - 총원가
            SELECT 49 rn, N'  VIII. 영업이익' gubun, M.구분, M.model,
        ...
        OP_MARGIN AS (
            -- VIII 영업이익률 = 영업이익 / 매출액
            SELECT 50 rn, N'    영업이익률' gubun, M.구분, M.model,
                   CAST(
...
--            UNION ALL SELECT rn, gubun, 구분, model, amt FROM MERCH_COGS    --26/02/13 KYH삭제         
        ...
        SELECT 구분, rn, gubun, model, pivot_key, amt
        INTO #BASE
        FROM BASE;
		...
		SET @SQL = N'
		;WITH P AS (
		    SELECT TRY_CONVERT(INT, rn) AS rn, gubun, ' + @Columns + N'
		    FROM (
		        SELECT TRY_CONVERT(INT, rn) AS rn, gubun, pivot_key, amt
		        FROM #BASE
		    ) S
		...
		LEFT JOIN P Op   ON Op.rn   = 49 -- 영업이익  : Operating Profit
		LEFT JOIN P Bep   ON Bep.rn   = 53 -- 손익분기점 : Break-Even Point (BEP) Operating Profit
