   ...
   --데이타 체크
	...
	MODEL_STOCK AS (
	    SELECT
	        ...
	        SUM(A.OUT /*- OUT_ETC*/) AS OUT,
	        SUM(A.EOH) AS EOH,
	        SUM(A.INPUT_ETC) AS INETC,
	        SUM(A.OUT_ETC) AS OUTETC
	    FROM DOI_STOCK a
	    WHERE a.YYYYMM = @YYYYMM
	      AND a.SITE = @SITE
	    GROUP BY 
	        ...
	        a.SITE,
	        A.MODEL,
	        CASE WHEN RIGHT(a.model_type, 1) = 'P' THEN '양산' ELSE '개발' END
	),
	MODEL_EOH_AMT AS (
	    SELECT 
	        YYYYMM,
	    ...
	    FROM (
	        SELECT 
	            ...
	            Base_EOH_AMT + CASE WHEN rn_eoh = 1 THEN ROUND(Ori_total, 0) - Base_total ELSE 0 END AS Final_Eoh
	        FROM (
	            SELECT 
	                a.*,
	                -- expen_sel 기준 파티셔닝 제거 (MODEL_STOCK에 expen_sel 정보 없음)
	                ROW_NUMBER() OVER (PARTITION BY YYYYMM, SITE, SEL_CODE, 구분, MODEL ORDER BY Ori_OUTETC_amt DESC) AS RN_OUTETC,
	                ROW_NUMBER() OVER (PARTITION BY YYYYMM, SITE, SEL_CODE, 구분, MODEL ORDER BY Ori_Eoh_amt DESC) AS RN_Eoh,
	                SUM(Ori_OUTETC_amt) OVER (PARTITION BY YYYYMM, SITE, SEL_CODE, 구분, MODEL) AS Ori_OUTETC_total,
	                SUM(Base_OUTETC_amt) OVER (PARTITION BY YYYYMM, SITE, SEL_CODE, 구분, MODEL) AS Base_OUTETC_total,
	            ...
	            FROM (
	                SELECT
	                    ...
	                    a.구분,
	                    A.MODEL,
	                    b.expen_sel,  -- MODEL_BOH_AMT에서 가져옴
	                    b.expen_sel명,
	                    b.acct_name,
	                    A.BOH,
	                    A.INPUT,
	                    ...
	                    B.INETC_AMT,
	                    --B.OUTETC_AMT,
	                    CASE WHEN BOH+INPUT = OUTETC THEN  (COALESCE(b.boh_amt, 0) + COALESCE(c.IN_AMT, 0))
	                    ...
	                    CASE WHEN BOH+INPUT = EOH THEN  (COALESCE(b.boh_amt, 0) + COALESCE(c.IN_AMT, 0))
	                    	 ELSE (COALESCE(b.boh_amt, 0) + COALESCE(c.IN_AMT, 0))/(COALESCE(A.boh, 0) + COALESCE(a.INPUT, 0))*A.EOH END AS Ori_EOH_AMT,
	                    CASE WHEN BOH+INPUT = EOH THEN  (COALESCE(b.boh_amt, 0) + COALESCE(c.IN_AMT, 0))
	                    	 ELSE round( (COALESCE(b.boh_amt, 0) + COALESCE(c.IN_AMT, 0))/(COALESCE(A.boh, 0) + COALESCE(a.INPUT, 0))*A.EOH ,0) END AS Base_EOH_AMT
	                   /* CASE 
	                ...
	                LEFT JOIN MODEL_IN_AMT c  
	                    ...
	                    AND a.site = c.site 
	                    AND a.model = c.model  
	                    AND a.구분 = c.구분 --CASE WHEN c.MAX_구분 = c.MIN_구분 THEN a.구분 ELSE c.구분 END 
	                    AND b.expen_sel = c.expen_sel  -- expen_sel은 B와 C 사이에서 JOIN
