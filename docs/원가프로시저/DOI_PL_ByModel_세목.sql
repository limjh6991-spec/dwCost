CREATE   PROCEDURE DOI_PL_ByModel_세목
(
    @YYYYMM VARCHAR(6),
    @SITE VARCHAR(4)
)
...
BEGIN
    ...
    BEGIN TRY
        ...
        , COGS_BASE AS (
            ...
            ------------------------------------------------------------------
            SELECT
            	CASE WHEN S.구분='카세트' THEN '양산' ELSE S.구분 END AS 구분
                , S.MODEL                                       AS model
...
--                    + ISNULL(S.IN_AMT,0)
--                    - ISNULL(S.EOH_AMT,0))
                , SUM(out_amt)    				AS prod_cogs_amt      -- 제품매출원가
            FROM DOI_STCO S
        WHERE YYYYMM = @YYYYMM
              AND S.SITE = @SITE
GROUP BY S.구분, S.MODEL
        ...
        , PL_HEAD AS (
            ...
            LEFT JOIN SALES_BASE S ON S.model = M.model and S.구분 = M.구분
            UNION ALL
            SELECT 4 rn, '    (3) 상품매출' gubun, M.구분, M.model, CAST(NULL AS DECIMAL(18,2)) FROM #MODEL M
            ...
            UNION ALL
            SELECT 6 rn, '    (1) 제품매출원가' gubun, M.구분, M.model, ISNULL(C.prod_cogs_amt,0) FROM #MODEL M LEFT JOIN COGS_BASE C ON C.model = M.model and C.구분 = M.구분 and C.구분 = M.구분
            UNION ALL
            ...
            ------------------------------------------------------------------
            UNION ALL
            SELECT 141 rn, '  V. 영업이익' gubun, M.구분, M.model, ISNULL(S.total_sale_amt,0) - ( ISNULL(C.prod_cogs_amt,0) + ISNULL(C.merch_cogs_amt,0) ) - ISNULL(G.sgna_amt,0) amt FROM #MODEL M
            LEFT JOIN SALES_BASE S ON S.model = M.model and S.구분 = M.구분
            LEFT JOIN COGS_BASE C ON C.model = M.model and C.구분 = M.구분
            LEFT JOIN SGNA_SUM G ON G.model = M.model and G.구분 = M.구분
        )
       ...
       -- 개발 모델 합계
        ...
        ----------------------------------------------------------------------
        SELECT top 100 
            --@Columns = COALESCE(@Columns + '], [', '') + model,
        	--@PivotColumns = COALESCE('coalesce(['+model +@PivotColumns + '],0) as ['+model + ']','')
        ...
        FROM (
            SELECT model mdl ,구분+model as model FROM #MODEL
            UNION ALL SELECT 'ZZZZ',N'Z합계'
            UNION ALL SELECT 'ZZZY',N'개발Z합계개발'            
            UNION ALL SELECT 'ZZZX',N'양산Z합계양산'
        ) A;
