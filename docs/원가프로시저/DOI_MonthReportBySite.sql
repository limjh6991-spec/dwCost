CREATE       Procedure DOI_MonthReportBySite
(
    @YYYY varchar(4),
    @SITE varchar(4)
)
...
ISNULL(SUM(b.원화판매금액계), 0) AS 금액
FROM #GEN_YYYYMM a 
LEFT JOIN DOI_SALE_RESC b ON (a.YYYYMM = b.YYYYMM AND b.SITE = @SITE AND RIGHT(b.품번, 1) != 'D')
GROUP BY a.YYYYMM, a.월번호
UNION ALL
-- 수량 (Cell)
SELECT 
...
FROM #GEN_YYYYMM a 
LEFT JOIN DOI_STOCK_COST b ON (a.YYYYMM = b.YYYYMM AND b.SITE = @SITE)
GROUP BY a.YYYYMM, a.월번호
UNION ALL
-- ========================================================================
-- 11. 기초제품재고액
...
FROM #GEN_YYYYMM a 
LEFT JOIN DOI_SALE_RESC b ON (a.YYYYMM = b.YYYYMM AND b.SITE = @SITE)
LEFT JOIN DOI_STOCK_COST c ON (a.YYYYMM = c.YYYYMM AND c.SITE = @SITE)
GROUP BY a.YYYYMM, a.월번호
UNION ALL
...
'영업이익' AS 구분,
CAST(a.월번호 AS VARCHAR) + '월' AS 월,
b.원화판매금액계 - c.OUT_AMT - d.DIST_AMT AS 금액
FROM #GEN_YYYYMM a 
LEFT JOIN (
SELECT a.yyyymm,
...
FROM #GEN_YYYYMM a
LEFT JOIN DOI_STOCK_COST c with (nolock) ON (a.YYYYMM = c.YYYYMM AND c.SITE = @SITE)
GROUP BY a.yyyymm
...
ELSE ((b.원화판매금액계 - c.OUT_AMT) - d.DIST_AMT) / NULLIF(b.원화판매금액계, 0) * 100
END AS 금액
FROM #GEN_YYYYMM a 
LEFT JOIN (
SELECT a.yyyymm,
